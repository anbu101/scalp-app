...
def frame_divider(factor: float) -> float:
    """Ratio separating the two frames — geometric midpoint of 1 and factor.

    MUST be used by every band so `native` and `scaled` stay DISJOINT. The
    first cut of this module used [LO,HI] and [factor*LO, factor*HI], which
    overlap on [factor*LO, HI] whenever factor*LO < HI — true for factor 2,
    since 1.20 < 1.45. Rows in that strip were counted twice: junk went
    negative and single-frame months were flagged DUAL. Fixed 2026-08-28.
    """
    return factor ** 0.5
...
def frame_scan(db: str, underlying: str, *, factor: float = 2.0) -> List[Dict]:
    ...
    args = (underlying, FRAME_LO, div, div, factor, FRAME_HI, underlying)
    orphan_sql = f"""
...
def repair_frame_split(db: str, underlying: str, *, ex_date: str,
    ...
    try:
        pre_del, pre_keep = conn.execute(f"""
            ...
            WHERE o.underlying=? AND o.instrument_type IN ('CE','PE')
              AND {_ODAY} < ?""",
            (u, div, factor, FRAME_HI, div, factor, FRAME_HI,
             u, ex_date)).fetchone()
        post_del, post_keep = conn.execute(f"""
            ...
            WHERE o.underlying=? AND o.instrument_type IN ('CE','PE')
              AND {_ODAY} >= ?""",
            (u, FRAME_LO, FRAME_HI, FRAME_LO, FRAME_HI, u, ex_date)).fetchone()
    ...
    try:
        ...
        conn.execute(f"""
            ...
            (u, u, ex_date, div, factor, FRAME_HI))
        conn.execute(f"""
            DELETE FROM backtest_candles_1m WHERE rowid IN (
              SELECT o.rowid FROM backtest_candles_1m o
              JOIN (SELECT {_DAY} d, avg(close) sp FROM backtest_candles_1m
