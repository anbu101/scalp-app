...
class AngelOneExecutor(BaseOrderExecutor):
    ...
    # --------------------------------------------------
    # REGULAR ORDERS — UNVERIFIED (WAVE 2). FAIL CLOSED.
    # --------------------------------------------------

    def _unverified(self, name: str):
        raise AngelUnverifiedEndpointError(
            f"AngelOneExecutor.{name} not live-verified yet (Wave 2 gate). "
            f"Strategy must remain PAPER on this account.")

    def place_buy(self, symbol: str, token: int, qty: int):
        self._unverified("place_buy")

    def place_sell_entry(self, symbol: str, token: int, qty: int):
        self._unverified("place_sell_entry")

    def place_buy_exit(self, symbol: str, qty: int, reason: str) -> str:
        self._unverified("place_buy_exit")

    def place_market_sell(self, symbol: str, qty: int) -> str:
        self._unverified("place_market_sell")

    def get_order_fill(self, order_id: str) -> Dict:
        self._unverified("get_order_fill")

    def get_last_avg_price(self, order_id: str) -> float:
        self._unverified("get_last_avg_price")

    def cancel_order(self, order_id: str):
        self._unverified("cancel_order")

    def get_orders(self) -> List[Dict]:
        self._unverified("get_orders")

    def get_open_positions(self) -> List[Dict]:
        self._unverified("get_open_positions")

    def place_sl(self, symbol: str, qty: int, sl_price: float) -> str:
        self._unverified("place_sl")

    def place_exit(self, symbol: str, qty: int, reason: str):
        self._unverified("place_exit")
