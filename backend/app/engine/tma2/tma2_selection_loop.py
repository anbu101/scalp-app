# backend/app/engine/tma2/tma2_selection_loop.py
#
...
#
# BOOT: 3-session EMA warmup via Kite historical (tma2_live_warmup, fail
...
from app.engine.tma2.tma2_common import (STRATEGY_ID, TMA2Repo, hm_to_min,
                                       ist_day_start)
from app.engine.tma2.tma2_live_signal_engine import TMA2LiveSignalEngine
from app.engine.tma2.tma2_live_warmup import (WARMUP_DAYS, fetch_today_spot,
...
from app.engine.tma2.tma2_gtt_monitor import TMA2GTTMonitor
...
def get_manager() -> Optional[TMA2TradeManager]:
    return _RUNTIME.get("manager")


def get_signal_engine() -> Optional[TMA2LiveSignalEngine]:
    return _RUNTIME.get("engine")


class TMA2MinuteCoordinator:
    def __init__(self, signal_engine: TMA2LiveSignalEngine,
                 manager: TMA2TradeManager, exit_min: int, notify=None):
    ...
    def on_minute(self, ts: int, spot_candle: Optional[dict], chain) -> None:
        ...
        if in_session and self._spot_seen_count >= 5 and self.last_spot_seen \
            ...
            write_audit_log("[TMA2][WATCHDOG] no spot candle for 3+ minutes "
                            "during session — possible zombie WebSocket")
            if self.notify:
                try:
                    self.notify("TMA2: no spot candle for 3+ minutes — "
                                "possible zombie WebSocket, check the app")
                except Exception:
                    pass


async def tma2_selection_loop(zerodha_manager):
    ...
    last_run_day = None
    while True:
        armed_day = await wait_for_arm_window("TMA2", last_run_day)   # own log tag
        try:
            await _tma2_selection_loop_inner(zerodha_manager)
        except Exception as e:
            import traceback
            write_audit_log(f"[TMA2][CRITICAL] selection loop DIED: {e!r}\n"
                            f"{traceback.format_exc()}")
            try:
                from app.api.telegram_api import notify_system_alert
                notify_system_alert({"message": f"🚨 TMA2 loop DIED: {e!r} — "
                                                f"no TMA2 trading until "
...
async def _tma2_selection_loop_inner(zerodha_manager):
    ...
    try:
        ...
        capture_dir = str(APP_HOME / "tma2_capture")
    except Exception:
        import os
        capture_dir = os.path.expanduser("~/.scalp-app/tma2_capture")

    write_audit_log("[TMA2] selection loop starting")
    ...
    while kite is None:
        ...
        if kite is None:
            if _waited and _waited % 300 == 0:
                write_audit_log(f"[TMA2] waiting for Zerodha session "
                                f"({_waited // 60} min)")
    ...
    instruments_df = None
    # ── TMA_V2 ── config is read BEFORE the engine is built: the frozen
    # study parameters (exit ref, slope gate, max extension) are
    # constructor args, not per-call args. They are config keys with
    # locked defaults and are deliberately absent from the Settings UI
    # (LD2) — only max_extension_pct is user-facing.
    _boot_cfg = load_strategy_config(STRATEGY_ID) or {}
    sig_engine = TMA2LiveSignalEngine(
        xover_exit_ref=int(_boot_cfg.get("xover_exit_ref", 55) or 55),
        max_extension_pct=float(_boot_cfg.get("max_extension_pct", 0.8) or 0),
        slope_gate=bool(_boot_cfg.get("ema144_slope_gate", True)),
    )
    _boot_attempts = 0
    while True:
        ...
        if _ist_min >= 15 * 60:
            if _boot_attempts == 0:
                ...
                write_audit_log("[TMA2] launched after the boot cutoff (15:00 "
                                "IST) — market over; idle until next session "
                                "re-arm (DAY_CYCLE)")
            else:
                write_audit_log(f"[TMA2] boot never succeeded before 15:00 IST "
                                f"({_boot_attempts} attempts) — giving up for "
                                f"today (fail closed)")
                if notify:
                    try:
                        notify(f"TMA2: boot kept failing ({_boot_attempts} "
        ...
        try:
            ...
            # ── TMA2_XDAY_WARMUP ── FIVE sessions (EMA144 seed depth)
            ...
            write_audit_log(f"[TMA2][BOOT] attempt failed ({e!r}) — retrying in 60s")
    ...
    if not (sess_start_min < sess_end_min <= exit_min):
        write_audit_log(f"[TMA2][ABORT] session window invalid: "
                        f"{cfg.get('session_start')} < {cfg.get('session_end')} "
                        f"<= {cfg.get('exit_time')} must hold — not trading")
        if notify:
            try:
                notify("TMA2: invalid session window in Settings — not trading "
    ...
    if _now_ist_min > (9 * 60 + 16):
        ...
        for _c in _bf:
            ...
            _fed += 1
        if _fed:
            write_audit_log(f"[TMA2][BACKFILL] mid-session start: {_fed} spot "
                            f"candles restored (last ts {_bf[-1]['ts']})")
        else:
            write_audit_log("[TMA2][BACKFILL] mid-session start but no candles "
    ...
    try:
        ...
        executor = get_executor_for_strategy('TMA_V2')
        write_audit_log("[TMA2] executor pre-built")
    except Exception as e:
        write_audit_log(f"[TMA2] executor build failed ({e!r}) — PAPER "
                        f"unaffected; LIVE entries will be skipped (fail closed)")

    repo = TMA2Repo()
    manager = TMA2TradeManager(cfg, repo, executor=executor, notify=notify)
    ...
    if trade_mode == "INTRADAY" and prior:
        ...
        write_audit_log(f"[TMA2] {len(prior)} OPEN leg(s) from a previous "
                        f"session marked STALE (INTRADAY) — review manually")
        if notify:
            try:
                notify(f"TMA2: {len(prior)} stale open leg(s) from a previous "
    ...
    if manager.disabled:
        write_audit_log("[TMA2] manager disabled by reconciliation — loop exiting")
        return

    _RUNTIME["manager"] = manager
    _RUNTIME["engine"] = sig_engine
    coord = TMA2MinuteCoordinator(sig_engine, manager, exit_min, notify=notify)

    # ── universe + websocket ──
    engine = TMA2TickEngine(zerodha_manager, instruments_df,
                           coord.on_minute, capture_dir=capture_dir)
    try:
        spot_ltp = float(kite.ltp(["NSE:NIFTY 50"])["NSE:NIFTY 50"]["last_price"])
    except Exception as e:
        write_audit_log(f"[TMA2] spot LTP fetch failed ({e}) — not trading (fail closed)")
        return
    expiry_iso = expected_expiry_for_day(today).isoformat()
    n = engine.resolve_universe(spot_ltp, expiry_iso)
    if n == 0:
        write_audit_log("[TMA2] empty option universe — not trading (fail closed)")
        if notify:
            try:
                notify("TMA2: empty weekly option universe at boot — verify "
    ...
    if executor is not None:
        mon = TMA2GTTMonitor(executor, manager)
        mon.start()
        _RUNTIME["monitor"] = mon

    write_audit_log(f"[TMA2] LIVE loop up: {n} contracts, expiry {expiry_iso}, "
                    f"warmup {sig_engine.diag['warmup_sessions']}/{WARMUP_DAYS} "
                    f"session(s), mode={cfg.get('trade_execution_mode')}, "
                    f"trade_mode={trade_mode}, "
                    f"exitRef EMA{sig_engine.xover_ref}, "
                    f"maxExt {sig_engine.max_ext}%, "
                    f"slope {'ON' if sig_engine.slope_gate else 'OFF'} "
                    f"(SELL-spread only)")
    ...
    try:
        ...
        write_audit_log(f"[TMA2][DAY_CYCLE] engine.stop failed: {e!r}")
    _mon = _RUNTIME.get("monitor")
    if _mon is not None:
        try:
            _mon.stop()
        except Exception as e:
            write_audit_log(f"[TMA2][DAY_CYCLE] monitor.stop failed: {e!r}")
    _RUNTIME["manager"] = None
    _RUNTIME["engine"] = None
    _RUNTIME["monitor"] = None
    write_audit_log("[TMA2][DAY_CYCLE] day complete — torn down; will re-arm "
                    "next session")
