# backend/app/engine/tma2/tma2_common.py
#
...
# stray-parallel-DB incident — NEVER hand-build the path). Table tma2_trades
...
# ── TMA_V2 LIVE ── four-EMA STACK (13/55/89/144 @5m), SELL-spread only.
...
class TMA2Repo:
    """Thin sqlite repo for tma2_trades. WAL-friendly, short-lived
    ...
    def ensure_schema(self) -> None:
        try:
            ...
            write_audit_log(f"[TMA2][DB] ensure_schema failed: {e}")
    ...
    def update_leg(self, leg_db_id: int, **fields) -> None:
        ...
        try:
            ...
            write_audit_log(f"[TMA2][DB] update_leg({leg_db_id}) failed: {e}")
    ...
    def mark_stale(self, leg_db_id: int) -> None:
        ...
        try:
            ...
            write_audit_log(f"[TMA2][DB] mark_stale({leg_db_id}) failed: {e}")
    ...
    def open_legs(self) -> List[dict]:
        try:
            ...
            write_audit_log(f"[TMA2][DB] open_legs failed: {e}")
            return []