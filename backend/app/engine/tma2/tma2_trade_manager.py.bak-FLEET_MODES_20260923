# backend/app/engine/tma2/tma2_trade_manager.py
#
# ── TMA_V2 TRADE MANAGER ── (paper parity + Session-C live hardening)
# ============================================================================
# ── TMA_V2 ── One open GROUP at a time, EITHER direction (backtest D2:
# a single S1 slot shared by E1 and E2 — unlike V1's per-condition slots):
...
# tma2_trades linked by group_id, direction SELL|BUY, mode stamped per entry.
#
...
# (parity-proven by test_tma2_live_core); LIVE fills are real (divergence
...
#
# RESTART ADOPTION: OPEN tma2_trades rows on boot — INTRADAY: prior-day rows
...
try:
    from app.engine.tma2.tma2_common import (STRATEGY_ID, TABLE, TMA2Repo,
                                           hm_to_min, ist_day_start, leg_net)
    from app.engine.tma2.tma2_live_core import (hard_close,
                                              mtm_cut_after_data_gap,
                                              new_position, sltp_levels,
                                              step_candle)
except ImportError:  # standalone tests
    from tma2_common import (STRATEGY_ID, TABLE, TMA2Repo,      # type: ignore
                            hm_to_min, ist_day_start, leg_net)
    from tma2_live_core import (hard_close, mtm_cut_after_data_gap,  # type: ignore
...
class TMA2TradeManager:
    ...
    def __init__(self, cfg: dict, repo: TMA2Repo, executor=None, notify=None):
        self.disabled = False
        self.repo = repo
    ...
    def _cfg_snapshot(self) -> Optional[dict]:
        ...
        # ── TMA_V2 CONFIG SHAPE ── s1.main (the SOLD, monitored leg) and
        # s1.hedge, mirroring the backtest runner's config exactly. The
        # internal snapshot keys keep the sell_*/buy_* names because in
        # SELL mode the main leg IS the sold leg (V2 is SELL-only live).
        s1 = cfg.get("s1") or {}
        sell = dict(s1.get("main") or {})
        buy = dict(s1.get("hedge") or {})
        ...
        snap = {
            ...
            "sell_premium_max": float(sell.get("premium_max", 0) or 0),   # ── FAIL_CLOSED_LOTS ──
            "sell_lots": int(sell.get("lots", 0) or 0),   # ── FAIL_CLOSED_LOTS ──
            ...
            "buy_premium_max": float(buy.get("premium_max", 0) or 0),     # ── FAIL_CLOSED_LOTS ──
            "buy_lots": int(buy.get("lots", 0) or 0),     # ── FAIL_CLOSED_LOTS ──
            "max_tpd": int(s1.get("max_trades_per_day", 0) or 0),
        }
        ...
        if mode == "LIVE":
            if self.executor is None:
                ...
                record_alert("TMA_NO_EXECUTOR",
                             "TMA_V2: LIVE mode but broker executor missing — entry skipped",
    ...
    def _build_group(self, *, sig, snap, mode, sell_sym, hedge_sym,
        ...
        group = {
            # ── E1/E2 ── the stack condition travels ON THE GROUP: sig is
            # only in scope here, and _persist_entry / adoption both need it.
            "cond": str(sig.get("cond") or "E1"),
    ...
    def _persist_entry(self, group: Dict) -> None:
        ...
        common = dict(group_id=g["group_id"], mode=g["mode"],
                      ...
                      condition=str(g.get("cond") or "E1"))   # ── E1/E2 ──
    ...
    def _enter_live(self, sig, snap, chain, sell_sym, hedge_sym, sell_qty,
        ...
        if snap["margin_guard"] and not self._margin_ok(
            ...
            record_alert("TMA_MARGIN_BLOCK",
                         "TMA_V2 entry blocked: margin shortfall (< required × 1.25)",
                         severity="error", strategy_id=self._sid, mode="live")
            try:
                notify_critical({"message": "TMA_V2: entry SKIPPED — available "
        ...
        if h_fill is None:
            ...
            record_alert("TMA2_ENTRY_FAIL", "TMA_V2: hedge BUY never filled — no entry",
    ...
    def _unwind_hedge(self, hedge_sym: str, qty: int, entry_fill: float,
                      cond: str = "E1") -> None:
        ...
        db_id = self.repo.insert_leg({
            ...
            "condition": str(cond or "E1"),   # ── E1/E2 ── from the signal
            "status": "OPEN"})
        if db_id:
        ...
        record_alert("TMA_UNWOUND",
                     f"TMA_V2: SELL leg rejected after hedge fill — hedge "
                     f"{hedge_sym} unwound @{exit_px:.2f}",
                     severity="error", strategy_id=self._sid, mode="live")
        try:
            notify_critical({"message": f"TMA_V2: SELL entry FAILED after the "
        ...
        if oc is None:
            ...
            if self._sold_gap_streak >= 3 and not self._gap_alerted:
                ...
                record_alert("TMA_CANDLE_GAP",
                             f"TMA_V2: no candles for {g['sell']['symbol']} for "
    ...
    def _live_sl_detected(self, res: dict) -> None:
        ...
        if status == "triggered":
            ...
            record_alert("TMA_GTT_ALREADY_FIRED",
                         f"TMA_V2: SL GTT already executed at the broker — "
    ...
    def _exit_group(self, res: dict, *, forced: bool,
        ...
        if live and not sell_already_flat:
            ...
            if gid:
                ...
                if gone:
                    ...
                    record_alert("TMA_GTT_STUCK",
                                 f"TMA_V2 {reason} exit blocked: GTT {gid} "
            ...
            if observed is not None and observed >= 0:
                ...
                record_alert("TMA_BUYBACK_SKIPPED",
                             f"TMA_V2: {g['sell']['symbol']} was already flat "
                ...
                try:
                    notify_critical({"message":
                        f"TMA_V2 {reason}: {g['sell']['symbol']} already flat "
                ...
                try:
                    ...
                    try:
                        notify_critical({"message":
                            f"TMA_V2: buyback of {g['sell']['symbol']} FAILED at "
        ...
        if live:
            try:
                ...
                try:
                    notify_critical({"message":
                        f"TMA_V2: hedge sell of {g['hedge']['symbol']} FAILED "
    ...
    def adopt_rows(self, rows: List[dict], *, kite=None) -> None:
        """Rebuild the open group from OPEN tma2_trades rows (both legs of one
        ...
        if len(by_gid) > 1:
            ...
            record_alert("TMA_MULTI_GROUP",
                         f"TMA_V2: {len(by_gid)} open groups in tma2_trades — "
            ...
            try:
                ...
                if not want_ok:
                    ...
                    write_audit_log(f"[{self._sid}][RECON] broker positions "
                                    f"do not match tma2_trades rows — manager "
                                    f"DISABLED (fail closed), resolve manually")
                    try:
                        notify_critical({"message":
                            "TMA_V2 RECON MISMATCH: broker positions ≠ "
                            "tma2_trades open rows — TMA2 disabled, resolve "
