# backend/app/engine/ic_v1/ic_orphan_reconcile.py
#
# IC_V1 — ORPHAN ROW RECONCILER (2026-08-03)
# ============================================================================
...
# 15:25 paper sweep deliberately exempts IC_V1 (overnight carry). The
...
#   * PAPER rows: any OPEN paper_trades row for IC_V1 whose paper_trade_id
...
def reconcile_orphan_rows(gm) -> dict:
    ...
    if paper_closed:
        record_alert(
            "IC_ORPHAN_RECONCILE",
            f"IC_V1: closed {paper_closed} orphaned open PAPER row(s) at "
            f"entry price (rows with no owning group — pre-snapshot "
            f"restart artifacts).",
            severity="warning", strategy_id=STRATEGY_ID, mode="paper",
        )

    # ── LIVE: flag only, never touch ───────────────────────────────────
    try:
        from app.db.trades_repo import get_open_trades_for_strategy
        live_rows = get_open_trades_for_strategy(STRATEGY_ID) or []
        stray = []
        for r in live_rows:
        ...
        if stray:
            ...
            record_alert(
                "IC_ORPHAN_LIVE",
                f"IC_V1: {live_flagged} open LIVE row(s) with NO owning "
            ...
            try:
                ...
                notify_critical({"message":
                    f"IC_V1: {live_flagged} orphaned LIVE row(s) at boot "
