# backend/app.engine.scalp_v4.scalp_v4_selection_loop.py
#
# SCALP_V4 — Selection Loop (TEST option-BUYING hedge clone of SCALP_V1).
# ============================================================================
# Mirrors scalp_v2_selection_loop: builds — once — the SCALP_V4 tick engine,
...
#   - scalp_v4_live_eod.py  (EOD square-off)
#   - the V4 API / frontend  (dashboard state) [later]
#
# Isolated: builds SCALP_V4's OWN engine + OWN KiteTicker. SCALP_V1 / BB / HA /
# V2 untouched. Launched as a standalone async task by api_server (deferred from
# StrategyRuntimeManager), exactly like SCALP_V2.
#
# Executor: uses the ROUTER (get_executor_for_broker) — V4's manager targets the
...
#   RECHECK_INTERVAL, phase_offset=30) so SCALP_V1 and SCALP_V4 take their LTP
...
#   plain fixed sleeps — those are retries, not aligned cycles. V4 remains fully
...
from app.engine.scalp_v4.scalp_v4_tick_engine import ScalpV4TickEngine
...
_ENGINE: Optional[ScalpV4TickEngine] = None


def get_engine() -> Optional[ScalpV4TickEngine]:
    return _ENGINE
...
def _seconds_to_next_boundary(interval: int, phase_offset: int = 0) -> float:
    ...
    SCALP_V1 and SCALP_V4 take their LTP snapshots at the same instants (both
...
def _get_timeframe_sec() -> int:
    """SCALP_V4 uses 1-minute candles (same default as V1)."""
...
async def scalp_v4_selection_loop(broker_manager: ZerodhaManager, *args, **kwargs):
    """
    ...
    used; the SCALP_V4 strategy_id is fixed internally.
    """
    global _ENGINE
    ...
    if not license_state.is_usable():
        write_audit_log(
            f"[V4_SELECT] License not usable ({license_state.LICENSE_STATUS}) — engine not started"
        )
        return

    timeframe_sec = _get_timeframe_sec()
    write_audit_log(f"[V4_SELECT] Selection engine started ({STRATEGY_ID}) tf={timeframe_sec}s")

    while True:
        try:
            write_audit_log(f"[V4_SELECT] loop tick ({STRATEGY_ID})")

            if not broker_manager.is_ready():
                write_audit_log("[V4_SELECT] Broker not ready")
            ...
            if not kite_trade or not kite_data:
                write_audit_log("[V4_SELECT] Trade/Data session not ready")
                await asyncio.sleep(RECHECK_INTERVAL)
                continue
            ...
            if not instruments:
                write_audit_log("[V4_SELECT][ERROR] No instruments loaded")
                await asyncio.sleep(RECHECK_INTERVAL)
                continue
            ...
            write_audit_log(f"[V4_SELECT] Current weekly expiry = {current_weekly_expiry}")
            ...
            raw = selector.select()
            if not raw:
                write_audit_log("[V4_SELECT] selector returned empty")
                await asyncio.sleep(RECHECK_INTERVAL)
                continue
            ...
            if _ENGINE is None:
                ...
                if not universe:
                    write_audit_log("[V4_SELECT][FATAL] Weekly universe empty")
                    await asyncio.sleep(RECHECK_INTERVAL)
                    continue

                tokens = [o["instrument_token"] for o in universe]

                # Router executor (NOT raw) — V4 manager targets the router surface.
                executor = get_executor_for_broker("ZERODHA")

                engine = ScalpV4TickEngine(
                    kite_data=kite_data,
                ...
                _ENGINE = engine
                write_audit_log(
                    f"[V4_SELECT] Engine started tokens={len(tokens)} tf={timeframe_sec}s"
                )
            ...
            if final:
                ...
                write_audit_log(
                    f"[V4_SELECT] Updated selection ({STRATEGY_ID}): "
                ...
                try:
                    ...
                    write_audit_log(f"[V4_SELECT][SUBSCRIBE_ERR] {e}")

        except Exception as e:
            write_audit_log(f"[V4_SELECT] ERROR ({STRATEGY_ID}) {repr(e)}")

        # Steady-state cadence: snap to the shared wall-clock grid so V1 and V4
