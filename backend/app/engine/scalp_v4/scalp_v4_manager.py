# backend/app/engine/scalp_v4/scalp_v4_manager.py
#
...
#
# This MANAGER owns order placement + DB lifecycle for V4. The ENGINE
# (scalp_v4_engine.py) owns signal generation, pairing, and the tick-driven
# exit decision; it calls open_hedge_trade() / close_hedge_trade() here.
#
# NOTIFICATIONS:
#   V4 does NOT use TradeStateManager / PaperTradeRecorder, so it does not get
...
#   At most ONE OPEN V4 trade at a time, across CE and PE. The gate is checked
#   against the DB (get_open_v4_trade) so it survives restarts and cannot be
...
# this file touches only scalp_v4_repo + the execution router + config. No edits
...
# Shared trade notifications (in-app audio/toast + Telegram). V4 calls these
...
from app.db.scalp_v4_repo import (
    insert_v4_trade,
    confirm_hedge_fill,
    link_hedge_gtt,
    close_v4_trade,
...
class ScalpV4Manager:
    """
    One instance per process for SCALP_V4 (single-trade-global). Constructed by
    ...
    def _notify_entry(self, *, paper: bool, hedge_symbol: str, hedge_side: str,
        ...
        try:
            ...
            write_audit_log(f"[V4][NOTIFY][ENTRY_ERROR] {hedge_symbol} ERR={e}")
    ...
    def _notify_exit(self, *, paper: bool, hedge_symbol: str,
        ...
        try:
            ...
            # Map V4 exit reasons → the three exit notifiers:
            ...
            write_audit_log(f"[V4][NOTIFY][EXIT_ERROR] {hedge_symbol} ERR={e}")
    ...
    def _gate_open(self) -> Optional[dict]:
        """Return the open V4 trade row (DB-backed) if one exists, else None."""
        return get_open_v4_trade()
        ...
        *,
        ...
        with self._entry_lock:
            ...
            if existing is not None:
                write_audit_log(
                    f"[V4][SINGLE_TRADE_GATE] DROP signal={signal_symbol} "
                    f"hedge={hedge_symbol} — already OPEN id={existing.get('v4_trade_id')} "
        ...
        try:
            ...
            if not hedge_ltp or hedge_ltp <= 0:
                write_audit_log(
                    f"[V4][ENTRY_ABORT] hedge LTP unavailable for {hedge_symbol} "
                    f"— skipping entry"
                )
                return
            ...
            if paper:
                self._open_paper(
                    v4_id=v4_id, qty=qty, max_sl=max_sl, hedge_ltp=hedge_ltp,
    ...
    def _open_paper(self, *, v4_id, qty, max_sl, hedge_ltp,
        ...
        write_audit_log(
            f"[V4][PAPER][ENTRY] id={v4_id} signal={signal_symbol} "
    ...
    def _open_live(self, *, v4_id, qty, max_sl, hedge_ltp,
                   signal_symbol, signal_token, signal_side, signal_entry_price,
                   signal_sl, signal_tp, signal_candle_ts,
                   hedge_symbol, hedge_token, hedge_side):
        ...
        try:
            ...
            write_audit_log(f"[V4][LIVE][BUY_FAIL] {hedge_symbol} ERR={e}")
            return

        if filled_qty <= 0:
            write_audit_log(f"[V4][LIVE][BUY_ZERO_QTY] {hedge_symbol} — aborting")
            return
        ...
        write_audit_log(
            f"[V4][LIVE][ENTRY_PROVISIONAL] id={v4_id} signal={signal_symbol} "
        ...
        threading.Thread(
            ...
            args=(v4_id, str(order_id), hedge_symbol, filled_qty, max_sl),
            daemon=True,
            name=f"scalp-v4-fill-{v4_id[:8]}",
        ).start()

    def _confirm_fill_worker(self, v4_id, order_id, hedge_symbol, qty, max_sl):
        start = time.time()
        while time.time() - start < _ENTRY_FILL_CANCEL_S:
            try:
                ...
                write_audit_log(
                    f"[V4][LIVE][FILL_POLL_ERR] {hedge_symbol} order={order_id} ERR={e}"
                )
            ...
            if status == "COMPLETE" and avg > 0:
                self._on_hedge_filled(v4_id, hedge_symbol, float(avg), qty, max_sl)
                return
            if status in _DEAD_ORDER_STATUSES:
                self._on_hedge_dead(v4_id, hedge_symbol, status)
                return
            time.sleep(_ENTRY_FILL_POLL_INTERVAL_S)

        # Timeout → cancel unfilled entry.
        self._cancel_unfilled(v4_id, order_id, hedge_symbol)

    def _on_hedge_filled(self, v4_id, hedge_symbol, fill_price, qty, max_sl):
        row = get_v4_trade_by_id(v4_id)
        if not row or row.get("state") != "OPEN":
            write_audit_log(
                f"[V4][LIVE][FILL_STALE] id={v4_id} {hedge_symbol} "
        ...
        new_sl = confirm_hedge_fill(
            v4_trade_id=v4_id, fill_price=fill_price, max_sl_points=max_sl
        )
        write_audit_log(
            f"[V4][LIVE][FILL_CONFIRMED] id={v4_id} {hedge_symbol} "
        ...
        try:
            ...
            write_audit_log(
                f"[V4][LIVE][GTT_FAIL] {hedge_symbol} ERR={e} — hedge UNPROTECTED; "
        ...
        if not row or row.get("state") != "OPEN":
            write_audit_log(
                f"[V4][LIVE][GTT_RACE] id={v4_id} closed during GTT placement "
                f"— cancelling gtt_id={gtt_id}"
            )
            try:
                self.executor.cancel_gtt(gtt_id)
            except Exception as e:
                write_audit_log(f"[V4][LIVE][GTT_RACE_CANCEL_WARN] {hedge_symbol} ERR={e}")
            return

        link_hedge_gtt(v4_trade_id=v4_id, gtt_id=str(gtt_id))
        write_audit_log(
            f"[V4][LIVE][ENTRY_CONFIRMED] id={v4_id} {hedge_symbol} "
            f"entry={fill_price} hedge_sl={new_sl} gtt={gtt_id}"
        )

    def _on_hedge_dead(self, v4_id, hedge_symbol, status):
        row = get_v4_trade_by_id(v4_id)
        if not row or row.get("state") != "OPEN":
            return
        write_audit_log(
            f"[V4][LIVE][DEAD_ENTRY] id={v4_id} {hedge_symbol} status={status} "
            f"— no position opened, closing row"
        )
        close_v4_trade(
            v4_trade_id=v4_id, exit_price=None,
    ...
    def _cancel_unfilled(self, v4_id, order_id, hedge_symbol):
        row = get_v4_trade_by_id(v4_id)
        if not row or row.get("state") != "OPEN":
            return
        write_audit_log(
            f"[V4][LIVE][ENTRY_TIMEOUT] id={v4_id} {hedge_symbol} order={order_id} "
            f"unfilled after {_ENTRY_FILL_CANCEL_S}s — cancelling"
        )
        try:
            self.executor.cancel_order(order_id)
        except Exception as e:
            write_audit_log(f"[V4][LIVE][CANCEL_WARN] {hedge_symbol} ERR={e}")
        ...
        if status == "COMPLETE" and filled_qty >= qty and avg > 0:
            write_audit_log(
                f"[V4][LIVE][CANCEL_RACE_FILLED] {hedge_symbol} fill={avg} "
        ...
        write_audit_log(
            f"[V4][LIVE][ENTRY_CANCELLED] {hedge_symbol} clean cancel — closing row"
        )
        close_v4_trade(
            v4_trade_id=v4_id, exit_price=None,
    ...
    def close_hedge_trade(self, *, v4_trade_id: str, exit_reason: str):
        """
        ...
        """
        row = get_v4_trade_by_id(v4_trade_id)
        if not row or row.get("state") != "OPEN":
            write_audit_log(
                f"[V4][CLOSE_SKIP] id={v4_trade_id} reason={exit_reason} — not OPEN"
            )
            return
        ...
        # Capture entry now (for P&L in the notification) — close_v4_trade does
        ...
        if paper:
            close_v4_trade(
                v4_trade_id=v4_trade_id,
                exit_price=float(exit_price) if exit_price else None,
                exit_order_id=None, exit_reason=exit_reason,
            )
            write_audit_log(
                f"[V4][PAPER][EXIT] id={v4_trade_id} {hedge_symbol} "
        ...
        if hedge_gtt_id:
            ...
            try:
                ...
                write_audit_log(
                    f"[V4][LIVE][GTT_CANCEL_WARN] id={v4_trade_id} "
                    f"gtt={hedge_gtt_id} ERR={e} — proceeding to verify"
                )
            if not gone:
                write_audit_log(
                    f"[V4][LIVE][GTT_ORPHAN] id={v4_trade_id} gtt={hedge_gtt_id} "
                ...
                try:
                    ...
                    notify_critical({
                        "message": (
                            f"SCALP_V4 GTT {hedge_gtt_id} for {hedge_symbol} could NOT be "
        ...
        try:
            ...
            write_audit_log(
                f"[V4][LIVE][POS_CHECK_ERR] {hedge_symbol} ERR={e} — assuming open"
            )
            still_open = True
        ...
        if still_open:
            try:
                ...
                write_audit_log(
                    f"[V4][LIVE][EXIT_SELL_FAIL] {hedge_symbol} ERR={e} "
            ...
            write_audit_log(
                f"[V4][LIVE][ALREADY_FLAT] {hedge_symbol} — GTT filled; "
        ...
        write_audit_log(
            f"[V4][LIVE][EXIT] id={v4_trade_id} {hedge_symbol} "
    ...
    # ==================================================
    # EOD SQUARE-OFF — called by scalp_v4_live_eod_job
    # ==================================================

    def eod_squareoff(self) -> int:
        """
        Close every OPEN V4 trade (paper + live) at EOD. Uses close_hedge_trade
        with reason EOD, which handles both modes and the cancel→verify→sell
        path for live. Returns count closed.
        """
        rows = get_all_open_v4_trades()
        if not rows:
            write_audit_log("[V4][EOD] No open trades")
            return 0
        write_audit_log(f"[V4][EOD] Squaring off {len(rows)} trade(s)")
        closed = 0
        for r in rows:
            try:
                self.close_hedge_trade(v4_trade_id=r["v4_trade_id"], exit_reason="EOD")
                closed += 1
            except Exception as e:
                write_audit_log(
                    f"[V4][EOD][ERROR] id={r.get('v4_trade_id')} ERR={e}"
                )
        return closed
    ...
    def _skip(self, reason: str, signal_symbol: str, hedge_symbol: str):
        write_audit_log(
            f"[V4][SKIP] reason={reason} signal={signal_symbol} hedge={hedge_symbol}"
        )