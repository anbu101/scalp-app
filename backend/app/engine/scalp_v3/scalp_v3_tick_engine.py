...
class ScalpV3TickEngine:
    ...
    def _register_signal_candidate(self, *, symbol, token, signal_side,
        ...
        if late:
            write_audit_log(
                f"[V3_ENGINE][SIG_ARB_LATE] {symbol} ({signal_side}) ts={candle_ts} "
        ...
        if arm:
            threading.Thread(
                ...
                args=(candle_ts,),
                daemon=True,
                name=f"scalp-v3-sigarb-{candle_ts}",
            ).start()
    ...
    def _arbitrate_after_window(self, candle_ts: int):
        ...
        if len(candidates) > 1:
            ...
            write_audit_log(
                f"[V3_ENGINE][SIG_ARB] candle_ts={candle_ts} {len(candidates)} signals "
    ...
    def _pair_and_enter(self, *, symbol, token, signal_side,
        ...
        if hedge is None:
            write_audit_log(
                f"[V3_ENGINE][NO_HEDGE] {symbol} but no "
                f"{'PE' if signal_side == 'CE' else 'CE'} selection — skipping (per spec)"
            )
            return

        write_audit_log(
            f"[V3_ENGINE][SELL_SIGNAL] {symbol} ({signal_side}) "
