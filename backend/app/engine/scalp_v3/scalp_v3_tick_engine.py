# backend/app/engine/scalp_v3/scalp_v3_tick_engine.py
#
# SCALP_V3 — Tick Engine (TEST option-BUYING hedge clone of SCALP_V1).
# ============================================================================
...
# so V3's ENTRY signals are the same as V1's.
#
# DIVERGENCE FROM V1/V2 (the whole point of V3):
#   The contract that FIRES the signal (e.g. 24500CE) is the "signal" contract:
#   TRACKED for its own SL/TP but NEVER traded. On a SELL signal, V3 BUYS the
...
#
# no_open_trade is ALWAYS True for the condition engine: V3's single-trade gate
# is DB-backed in the manager (scalp_v3_trades), NOT StrategyEngine.in_trade —
# StrategyEngine queries SCALP_V3 in trades/paper_trades, which V3 never writes,
# so its self-managed in_trade would be permanently wrong for V3. (This is a
...
#   so SELL signals fire for many contracts OUTSIDE V3's selected 2-CE / 2-PE
#   premium-band set. V1 rejects non-selected signals in SignalRouter
#   (CE_NOT_SELECTED / PE_NOT_SELECTED → EXIT). V3 has no router, so it gates in
#   _handle_signal: the SIGNAL contract MUST be one of V3's currently-selected
...
from app.engine.scalp_v3.scalp_v3_manager import ScalpV3Manager
...
class ScalpV3TickEngine:
    """
    SCALP_V3 WebSocket tick engine. Owns its own KiteTicker. Built by the
    selection loop with the shared data-kite, the option-universe tokens, and
    the execution router. Constructs its own ScalpV3Manager.
    """
    ...
    def __init__(
        ...
        write_audit_log(f"[V3_ENGINE][INDEX] index tokens resolved: {self.index_tokens}")
        ...
        write_audit_log(f"[V3_ENGINE] Current weekly expiry = {self.current_week_expiry}")
    ...
    def subscribe_additional_tokens(self, tokens: List[int]):
        ...
        def _do_subscribe():
            try:
                ...
                write_audit_log(f"[V3_ENGINE][WS] Additional tokens subscribed: {len(tokens)}")
            except Exception as e:
                write_audit_log(f"[V3_ENGINE][WS][ERROR] subscribe_additional_tokens: {e}")
    ...
    def _wait_and_connect(self):
        ...
        try:
            ...
            write_audit_log(f"[V3_ENGINE][WS][FATAL] kws.connect exception: {e}")
    ...
    def _on_connect(self, ws, response):
        ...
        write_audit_log(
            f"[V3_ENGINE][WS] Connected — subscribed {len(all_tokens)} tokens "
    ...
    def _on_close(self, ws, code, reason):
        write_audit_log(f"[V3_ENGINE][WS] Closed {code} {reason}")
        with self._lock:
            self._connected = False

    def _on_error(self, ws, code, reason):
        write_audit_log(f"[V3_ENGINE][WS] Error {code} {reason}")
    ...
    def _on_ticks(self, ws, ticks):
        ...
        for tick in ticks:
            ...
            try:
                ...
                write_audit_log(f"[V3_ENGINE][WATCH_ERROR] {symbol} {e}")
            ...
            def write_candle_async(
                ...
                try:
                    ...
                    # no_open_trade ALWAYS True — V3 gate is DB-backed in manager.
                    ...
                    write_audit_log(f"[V3_ENGINE][ERROR] Candle processing failed for {symbol}: {e}")

            threading.Thread(target=write_candle_async, daemon=True).start()

    # ==================================================
    # SIGNAL → HEDGE PAIRING
    # ==================================================
    ...
    def _handle_signal(self, *, symbol, token, entry_price, sl_price, tp_price, candle_ts):
        ...
        # means a pre-session or trade-off signal does NOT run _pick_hedge (which
        # would otherwise hit price lookups every candle and can hammer REST).
        try:
        ...
        if mode in ("CE", "PE") and mode != signal_side:
            write_audit_log(
                f"[V3_ENGINE][SIDE_BLOCKED] {symbol} side={signal_side} mode={mode} — drop"
            )
            return
        ...
        # symbols), so SELL signals fire for many contracts OUTSIDE V3's selected
        # 2-CE / 2-PE premium-band set. V1 rejects these in SignalRouter
        # (CE_NOT_SELECTED → EXIT); V3 has no router, so it MUST gate here, or it
        # trades out-of-band contracts (e.g. an 819-premium CE when the band is
        # 150–200) and diverges from V1. The SIGNAL contract must be one of V3's
        # currently-selected strikes on its own side.
        if not self._is_selected_signal(symbol, signal_side):
            write_audit_log(
                f"[V3_ENGINE][{signal_side}_NOT_SELECTED] {symbol} fired but is not in "
                f"V3's selected {signal_side} set — drop (out-of-band / non-selected)"
            )
            return
        ...
        if hedge is None:
            write_audit_log(
                f"[V3_ENGINE][NO_HEDGE] {symbol} fired but no "
                f"{'PE' if signal_side == 'CE' else 'CE'} selection — skipping (per spec)"
            )
            return

        write_audit_log(
            f"[V3_ENGINE][SELL_SIGNAL] {symbol} ({signal_side}) entry={entry_price} "
            f"sl={sl_price} tp={tp_price} → BUY hedge {hedge['symbol']} "
            f"({hedge['side']}, premium≈{hedge['premium']})"
        )
    ...
    def _is_selected_signal(self, symbol: str, signal_side: str) -> bool:
        """
        True iff `symbol` is one of V3's currently-selected strikes on its OWN
        side. This is the V3 analogue of V1's SignalRouter selection gate
        ...
        try:
            ...
            write_audit_log(f"[V3_ENGINE][SEL_READ_ERR] {e} — dropping signal")
            return False
    ...
    def _pick_hedge(self, *, opposite_of: str) -> Optional[Dict]:
        ...
        Price source order (avoids REST rate-limit storms):
          1. LTPStore  — the WS tick for this symbol. V3's own ticker already
        ...
        try:
            ...
            write_audit_log(f"[V3_ENGINE][PICK_HEDGE_READ_ERR] {e}")
            return None
    ...
    def _watch_exit(self, token: int, ltp: float):
        ...
        if token == int(row["signal_token"]):
            ...
            if sig_sl is not None and ltp >= float(sig_sl):
                write_audit_log(
                    f"[V3_ENGINE][SIG_SL] id={v3_id} signal={row['signal_symbol']} "
                    f"ltp={ltp} >= sl={sig_sl} — exit hedge"
                )
                self.manager.close_hedge_trade(v3_trade_id=v3_id, exit_reason="SIG_SL")
                return
            if sig_tp is not None and ltp <= float(sig_tp):
                write_audit_log(
                    f"[V3_ENGINE][SIG_TP] id={v3_id} signal={row['signal_symbol']} "
                    f"ltp={ltp} <= tp={sig_tp} — exit hedge"
                )
                self.manager.close_hedge_trade(v3_trade_id=v3_id, exit_reason="SIG_TP")
                return
        ...
        if paper and token == int(row["hedge_token"]):
            ...
            if hedge_sl is not None and ltp <= float(hedge_sl):
                write_audit_log(
                    f"[V3_ENGINE][HEDGE_SL][PAPER] id={v3_id} hedge={row['hedge_symbol']} "
                    f"ltp={ltp} <= hedge_sl={hedge_sl} — exit hedge"
                )
                self.manager.close_hedge_trade(v3_trade_id=v3_id, exit_reason="HEDGE_SL")
                return
