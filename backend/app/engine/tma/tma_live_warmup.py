# backend/app/engine/tma/tma_live_warmup.py
#
# ── TMA LIVE WARMUP ── (TMA_XDAY_WARMUP live analogue, Kite historical)
# ============================================================================
# EMA89@5m cannot warm up inside one session (~75 bars < 89-bar SMA seed).
# Boot-time fetch of the prior THREE trading sessions' 1m NIFTY spot, shaped
# exactly like the backtest corpus candles, plus each session's day_start —
# feeding TMALiveSignalEngine the same warmup_sessions list the backtest
...
IST = 5 * 3600 + 30 * 60
WARMUP_DAYS = 3            # backtest runner's WARMUP_DAYS — keep identical
...
def _prev_trading_days(kite, token: int, today: date, n: int) -> List[date]:
    try:
        ...
        write_audit_log(f"[TMA_WARMUP] prev-trading-days lookup failed: {e}")
        return []
...
def fetch_warmup_sessions(kite, *, today: Optional[date] = None,
    ...
    try:
        ...
        if not prevs:
            write_audit_log("[TMA_WARMUP] no previous trading days found — fail closed")
            return None
        ...
        for d in prevs:
            ...
            if not rows:
                write_audit_log(f"[TMA_WARMUP] empty minute data for {d} — skipping day")
                continue
            spot_1m = [{
        ...
        if not out:
            write_audit_log("[TMA_WARMUP] all warmup sessions empty — fail closed")
            return None
        write_audit_log(f"[TMA_WARMUP] {len(out)} prior session(s) fetched: "
                        + ", ".join(f"{d.isoformat()}" for d in prevs[-len(out):]))
        return out
    except Exception as e:
        write_audit_log(f"[TMA_WARMUP] unexpected failure: {e} — fail closed")
        return None
