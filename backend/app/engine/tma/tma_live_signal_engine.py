# backend/app/engine/tma/tma_live_signal_engine.py
#
# ── TMA LIVE SIGNAL ENGINE ── (PST parity doctrine, verbatim)
# ============================================================================
...
# pipeline — app.backtest.tma.tma_v1_engine (warmup_bars + aggregate +
# compute_state + build_signals) — over the day's growing candle prefix with
# the prior THREE sessions as warmup, exactly like backtest_tma_runner does
# per day (TMA_XDAY_WARMUP). Live signals therefore equal backtest signals
# BY CONSTRUCTION, not by careful porting. Compute cost: ~4 sessions of 5m
# bars (~300) through three EMAs, once per minute — low milliseconds.
#
...
# candles append. C1's transition definition is prefix-stable (it reads only
# bars i-1 and i plus a backward streak walk over COMPLETED bars), but the
# property is GUARDED at runtime anyway: any drift → the engine FREEZES
# (emits nothing, frozen=True) — fail closed, never trade unstable signals.
#
# XOVER SERVICE: the trade manager's crossover-reversal exit is evaluated on
# the SAME bars5/state this engine maintains, via the backtest's own
# xover_exit_ts — one indicator stream, zero drift between signal and exit.
#
...
try:
    from app.backtest.tma.tma_v1_engine import (
        build_signals, compute_state, warmup_bars, xover_exit_ts,
    )
...
class TMALiveSignalEngine:
    """One instance per trading day. Fail-closed everywhere: no warmup → no
    signals; instability → frozen; malformed candles → rejected."""

    def __init__(self, *, fast: int = 5, mid: int = 13, slow: int = 89,
                 tf_min: int = TF_MIN):
        self.fast, self.mid, self.slow = int(fast), int(mid), int(slow)
    ...
    def on_spot_candle(self, c: dict) -> List[dict]:
        ...
        bars5 = warm5 + today5
        state = compute_state(bars5, fast=self.fast, mid=self.mid,
                              slow=self.slow)
        res = build_signals(bars5, len(warm5), self._session0,
        ...
        for key, frozen_sig in self._emitted.items():
            ...
            if live_sig is None \
                ...
                write_audit_log(f"[TMA_LIVE][FATAL] prefix instability at "
        ...
        for key in sorted(current.keys()):
            ...
            # boundary) is FRESH: build_signals stamps ts_end = bar_ts+tf_s,
    ...
    def xover_ts_for(self, trend_side: str, after_ts: int) -> Optional[int]:
        ...
        signal's side, not the sold side) — the backtest's own xover_exit_ts
        over the exact bars/state stream the signals came from."""
        if self._state is None or not self._bars5:
            return None
        try:
            return xover_exit_ts(self._bars5, self._state, "C1", trend_side,
                                 int(after_ts), tf_s=self.tf_s)
        except Exception as e:
            write_audit_log(f"[TMA_LIVE] xover lookup failed: {e}")
            return None

    # ── parity-harness hook ─────────────────────────────────────────
    def reference_full_day(self) -> List[dict]:
        """build_signals over everything fed so far, one shot — what a
        ...
        today5 = [b for b in aggregate(self._candles, self.tf_min,
                                       self._day_start) if b["complete"]]
        res = build_signals(warm5 + today5, len(warm5), self._session0,
                            self._entry_start_ts, self._entry_end_ts,
                            tf_s=self.tf_s, fast=self.fast, mid=self.mid,
                            slow=self.slow)
        return res["signals"]
