...
DEFAULT_STRATEGY_CONFIGS = {
    ...
    # ==================================================
    # ── BRK_V1 BEGIN ──
    # Defaults are SEALED CONFIG A (bible 2026-09-02): net ₹12.04L /
    # DD ₹2.88L / net/DD 4.18 at 10 lots over 2020-08..2026-08; per lot
    # net ₹1.20L, DD ₹28.8K. Config B is s2_enabled=True (adds the 10:25
    # second window: +₹4.57L, DD ₹2.43L combined — S2 slice is 2020/2024-
    # weighted and NEGATIVE through 2026; see the bible's paper mandate).
    #
    ...
    #    buys at the decision instant and records the real fill (spread +
    #    slippage, usually adverse on a breakout candle).
    # 2. EXITS. Backtest SL/TP trigger on bar low/high and fill AT the
    #    level. Live SL+TP are ONE OCO GTT firing on ticks: an intraminute
    #    touch the 1m bar never printed can fire live, and fills slip.
    #    Same-bar SL+TP collisions resolved pessimistically (SL) in the
    #    backtest have no deterministic live equivalent.
    # 3. SELECTION. Backtest reads the close of the bar ending at 09:25;
    #    live samples chain LTPs at 09:25:00 — same instant, different
    #    transport; a stale quote on an illiquid strike can differ.
    # 4. CHARGES. Backtest uses the fleet charges model; live pays actual.
    "BRK_V1": {
        "trade_execution_mode": "PAPER",
        "underlying": "NIFTY",

        # ── sealed Config A (frozen; changing these leaves the seal) ──
        "select_time": "09:25",
        "select_below": 180,
        "select_min": 0,
        "break_above": 180,
        ...
        "sl_pts": 16,               # premium points below entry
        "tp_pts": 46,               # premium points above entry (0 = none)
        ...
        "s2_entry_last": "10:30",
        "s2_only_if_flat": True,    # never double capital (LD-sheet)
        "s2_only_if_loss": True,    # redeploy only after a losing morning

        # ── falsified levers kept OFF; runner supports them for research ──
        "trail_mode": "lock", "trail_trigger_pts": 0, "trail_lock_pts": 0,
        "trail_gap": 0, "time_stop_min": 0, "time_stop_need_pts": 0,
        "fallback_enabled": False, "fallback_min_pts": 0,
