...
def scalp_v4_live_eod_job():
    write_audit_log("[EOD][SCALP_V4] Square-off triggered")

    closed = 0
    try:
        manager = get_manager()
        if manager is None:
            write_audit_log("[EOD][SCALP_V4] No manager (engine not started) — nothing to do")
        else:
            closed = manager.eod_squareoff()
    except Exception as e:
        write_audit_log(f"[EOD][SCALP_V4][ERROR] {repr(e)}")

    # Daily reset of risk-alert keys (idempotent — other EOD jobs also call it).
    try:
        reset_strategy_risk_alerts()
    except Exception as e:
        write_audit_log(f"[EOD][SCALP_V4][RISK_RESET_ERR] {repr(e)}")

    write_audit_log("[EOD][SCALP_V4] Square-off complete")

    record_alert(
        "EOD_SQUAREOFF",
        f"SCALP_V4: end-of-day square-off complete — {closed} position(s) closed.",
        severity="info",
