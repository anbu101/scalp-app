# backend/app/jobs/scalp_v3_gtt_reconcile.py
"""
SCALP_V3 — Hedge GTT Reconciliation Loop
========================================
WHY THIS EXISTS
---------------
SCALP_V3 protects its LONG hedge with an SL-only GTT at the broker. When that
GTT fires, the broker sells the hedge — the position goes flat — but NOTHING in
the V3 engine notices on its own:
  ...
  - _watch_exit() in the tick engine handles the hedge's own SL only in PAPER
    ...
    contract's SL/TP.
  - The single-trade gate is the OPEN row in scalp_v3_trades. close_hedge_trade
...
held, and no new V3 trade can enter until the SIGNAL contract later happens to
...
This is the detector V3 was missing. V1 has reconcile_with_broker, V2 has its
GTT monitor, BB has GTTMonitor — V3 had none. This loop is the V3 analogue.

WHAT IT DOES (every RECONCILE_INTERVAL_SEC, LIVE only)
------------------------------------------------------
  1. get_manager() — the live manager, fresh each cycle (None before start).
  2. get_open_v3_trade(paper=False) — the single live open trade, if any.
...
LAUNCH
  Started by api_server as a standalone asyncio task next to the SCALP_V3
...
def _hedge_flat_at_broker(manager, hedge_symbol: str) -> bool:
    ...
    try:
        ...
        write_audit_log(
            f"[V3_RECON][POS_CHECK_ERR] {hedge_symbol} ERR={e} — treating as still open"
        )
        return False
...
def _reconcile_once() -> None:
    ...
    try:
        row = get_open_v3_trade(paper=False)
    except Exception as e:
        write_audit_log(f"[V3_RECON][OPEN_READ_ERR] {e} — skipping cycle")
        return
    if not row:
        return

    v3_id        = row.get("v3_trade_id")
    ...
    try:
        ...
        write_audit_log(
            f"[V3_RECON][GTT_FETCH_ERR] id={v3_id} gtt={hedge_gtt_id} ERR={e} "
    ...
    if g is None:
        ...
        if _hedge_flat_at_broker(manager, hedge_symbol):
            write_audit_log(
                f"[V3_RECON][GTT_ABSENT_FLAT] id={v3_id} hedge={hedge_symbol} "
                f"gtt={hedge_gtt_id} not in book AND position flat — closing as HEDGE_SL"
            )
            _safe_close(manager, v3_id, hedge_symbol)
        return
    ...
    if not _hedge_flat_at_broker(manager, hedge_symbol):
        write_audit_log(
            f"[V3_RECON][GTT_FIRED_BUT_POS_OPEN] id={v3_id} hedge={hedge_symbol} "
    ...
    write_audit_log(
        f"[V3_RECON][HEDGE_SL_DETECTED] id={v3_id} hedge={hedge_symbol} "
        f"gtt={hedge_gtt_id} status={status}, position flat — closing trade, freeing gate"
    )
    _safe_close(manager, v3_id, hedge_symbol)


def _safe_close(manager, v3_id: str, hedge_symbol: str) -> None:
    """
    ...
    """
    try:
        manager.close_hedge_trade(v3_trade_id=v3_id, exit_reason="HEDGE_SL")
    except Exception as e:
        write_audit_log(
            f"[V3_RECON][CLOSE_ERR] id={v3_id} hedge={hedge_symbol} ERR={e} "
            f"— will retry next cycle"
        )


async def scalp_v3_gtt_reconcile_loop():
    """
    Continuous ~10s poll that detects the hedge SL-only GTT firing in LIVE mode
    and closes the V3 trade so the single-trade gate is freed for the next
    signal. Started as a standalone asyncio task by api_server, next to the
    SCALP_V3 selection loop, under the same enabled + license gate.
    """
    # Yield immediately (Windows asyncio requirement; matches the selection loop).
    await asyncio.sleep(0)
    write_audit_log("[V3_RECON] Hedge-GTT reconciliation loop started (SCALP_V3)")
    ...
    while True:
        try:
            ...
            # loop die.
            write_audit_log(f"[V3_RECON][LOOP_ERR] {e!r}")
