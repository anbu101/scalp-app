...
async def _run_heavy_startup():
    ...
    with _boot_guard("scheduler"):
        ...
        scheduler.add_job(
            ...
            day_of_week="mon-fri", timezone="Asia/Kolkata")
        # ── VET_V1 END ──
        # ── BRK_V1 BEGIN ── 15:16 safety net ONE MINUTE after the engine's
        # own 15:15 EOD tick (the parity path). LD6: BRK is NOT overnight-
        # exempt — the generic 15:25 paper sweep is the second backstop for
        # PAPER rows; this job's real duty is LIVE rows (CRITICAL alert if
        # the runtime is unreachable — it has no executor of its own).
        # Unique id — a reused id silently replaces another strategy's cron.
        scheduler.add_job(
            brk_live_eod_job, trigger="cron", hour=15, minute=16,
            id="brk_live_eod_squareoff", replace_existing=True,
            day_of_week="mon-fri", timezone="Asia/Kolkata")
        # ── BRK_V1 END ──
    ...
    # ── BRK_V1 BEGIN ── 09:25 premium breakout scalp (sealed 2026-09-02,
    # strategy bible in docs/). Decisions ONLY at 1m closes via the sealed
    # backtest helpers (parity by construction); one trade per session, the
    # optional 10:25 second session is a SETTING (s2_enabled). Ships
    # mode=PAPER, lots=1. LIVE exits are ONE OCO GTT with cancel-verified
    # on every engine exit. Own _boot_guard: a BRK_V1 launch failure must
    # never abort the launches that follow it.
    with _boot_guard("launch BRK_V1"):
        if STRATEGIES.get("BRK_V1", {}).get("enabled", False) and \
                license_state.license_allows_strategy("BRK_V1"):
            _supervise(asyncio.create_task(brk_v1_runtime(zerodha_manager)), "brk_v1_runtime")
            write_audit_log("[SYSTEM] BRK_V1 standalone runtime launched")
    # ── BRK_V1 END ──
