# backend/app/api/tma_state_routes.py
#
# ── TMA STATE API ── read-only, feeds the TMA dashboard panel + curl-level
...
@router.get("/trades")
def tma_trades(limit: int = Query(200, le=1000), status: str = Query("all")):
    repo = _repo()
    try:
        import sqlite3
        with sqlite3.connect(repo.db_path, timeout=30) as c:
            c.row_factory = sqlite3.Row
            q = "SELECT * FROM tma_trades"
