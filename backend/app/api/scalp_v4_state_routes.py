# backend/app/api/scalp_v4_state_routes.py
#
# SCALP_V4 panel state — purpose-built for ScalpV4Panel.
#
# Unlike SCALP_V1 (4 independent slots), SCALP_V4 has at most ONE open trade,
...
# }
#
# Isolated: reads only scalp_v4_repo + selection files + config. No other
# strategy is touched. If V4 has never run (no table), returns empty/flat.
...
router = APIRouter(tags=["scalp-v4"])

STRATEGY_ID = "SCALP_V4"


@router.get("/api/scalp_v4/state")
def get_scalp_v4_state():
    try:
    ...
    try:
        ...
        write_audit_log(f"[API][V4_STATE][SEL_ERR] {e}")
        sel = {"CE": [], "PE": []}
    ...
    try:
        ...
        if row:
            open_trade = {
                "v4_trade_id": row.get("v4_trade_id"),
        ...
        write_audit_log(f"[API][V4_STATE][OPEN_ERR] {e}")
