...
class RunMetaRequest(BaseModel):   # ── RUN_FAVOURITES_20260830 ──
        ...
        try:
            ...
            with audit_muted():
                if req.strategy_id in ("BB_V1", "BB_V2"):
                    ...
                    # ── TMA_V1 END ──
                elif req.strategy_id == "TMA_V2":
                    # ── TMA_V2 BEGIN ── four-EMA STACK (13/55/89/144 @5m)
                    # spot signals; option BUY (trend side) or SELL opposite
                    # + hedge (V1 SPREAD mechanics); ONE slot; optional
                    # 13/89 crossover exit. Keep this chain in sync with
                    ...
                    result = {
                        "run_id": tma2["run_id"], "summary": tma2["summary"],
                        "config": tma2.get("config", (req.config_override or {})),
                        "trades": tma2["trades"], "strategy_id": req.strategy_id,
                    ...
                    # ── VAP_V1 END ──
                elif req.strategy_id == "BRK_V1":
                    # ── BRK_V1_20260830 BEGIN ── 09:25 premium breakout scalp:
                    ...
                    # breakout: touch of the ORB high/low buys the
                    # continuation side (CE up / PE down) at the next 1m
                    # open; CBO premium target, spot range/points stop plus
                    # optional premium stop, re-arm after a tf close back
                    # inside, max 2/day. Keep this chain in sync with
                    # queue_worker._dispatch_run_impl — two hand-maintained
                    # copies.
                    from app.utils.app_paths import APP_HOME
                    ...
                    result = {
                        "run_id": orb["run_id"], "summary": orb["summary"],
                        "config": orb.get("config", (req.config_override or {})),
                        "trades": orb["trades"], "strategy_id": req.strategy_id,
                        # ── ABORT_REASON_PASSTHROUGH ── see the TMA block
                        "aborted": orb.get("aborted"), "reason": orb.get("reason"),
                    }
                    # ── ORB_V1_20260903 END ──
...
def run_meta(run_id: str, req: RunMetaRequest):
    # ── RUN_FAVOURITES_20260830 ── favourite star + free-text note on a run. Partial
