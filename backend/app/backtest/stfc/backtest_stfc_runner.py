...
#
# Fence: STFC_OPT_20260913  (v2: STFC_SPOT_20260913 — spot-basis exits)
#
...
#       before target, both before time). exit_basis selects the level:
#       "premium" (default v1):
...
#       "spot" (STFC_SPOT_20260913 — the TradingView test's semantics):
#         1. SPOT stop (abs = index points | pct of entry spot; CE below /
#            PE above entry_spot). Trigger "touch" (1m low/high) books on
#            the OPTION's own 1m bar that minute — spot_sl_fill "low"
#            (default, pessimistic bound on a spot-triggered market sell)
#            or "close" (legacy). Trigger "close" sells at the NEXT
#            minute's option open (wick-immune, ORB_SLTRIG convention).
#         2. SPOT target (CE above / PE below) → spot_tp_fill "close"
#            (default), "high" (optimistic) or "low" (stress) of the
#            option's bar that minute; close-trigger → next open.
...
try:
    from app.backtest.stfc.stfc_v1_engine import (
        ...
        prem_level, prem_fill, spot_level, spot_breached, strike_step,
        atm_strike, SESSION_OPEN_MIN)
...
DEFAULTS: dict = {
    ...
    # ── exits (R3) ──
    "exit_basis": "spot",              # premium | spot   ── STFC_SPOT_20260913 ──
    "sl_mode": "pct",                  # off | abs | pct (premium: ₹/% of entry premium; spot: pts/% of entry spot)
    "sl_value": 0.1,
    "tp_mode": "pct",                  # off | abs | pct
    "tp_value": 0.3,
    "spot_sl_trigger": "touch",        # touch | close      (spot basis only)
    "spot_sl_fill": "low",             # low | close        (touch trigger only)
    "spot_tp_fill": "close",           # close | high | low (touch trigger only)
...
class STFCTrade:
    ...
    entry_ts: int
    entry_price: float
    sl: Optional[float]                # stop level (premium or spot per exit_basis)
    tp: Optional[float]                # target level (premium or spot per exit_basis)
...
def _merge_cfg(override: Optional[dict]) -> dict:
    ...
    # ── STFC_SPOT_20260913 ── a saved v1 config (has st_len but no
    # exit_basis) reproduces as PREMIUM basis; a blank override takes the
    # new default (spot); anything explicit wins.
    if "exit_basis" in (override or {}):
        cfg["exit_basis"] = "spot" if str(override["exit_basis"]).lower() == "spot" else "premium"
    elif override and "st_len" in override:
        cfg["exit_basis"] = "premium"
    else:
        cfg["exit_basis"] = DEFAULTS["exit_basis"]
    cfg["spot_sl_trigger"] = ("close" if str(cfg.get("spot_sl_trigger", "touch")).lower() == "close" else "touch")
    cfg["spot_sl_fill"] = ("close" if str(cfg.get("spot_sl_fill", "low")).lower() == "close" else "low")
    _tf = str(cfg.get("spot_tp_fill", "close")).lower()
    cfg["spot_tp_fill"] = _tf if _tf in ("close", "high", "low") else "close"
...
def _impl(*, db_path, strategy_id, underlying, date_from, date_to,
    ...
    for k in ("sl", "tp"):
        ...
        # ── STFC_SPOT_20260913 ── footgun guards: a premium-scale number
        # left behind after switching the basis must ABORT, never run.
        if cfg["exit_basis"] == "spot" and cfg[f"{k}_mode"] == "pct" and cfg[f"{k}_value"] > 5:
            return _abort(cfg, strategy_id,
                          f"spot basis: {k}_value {cfg[f'{k}_value']}% of spot looks like a "
                          f"premium % — use <= 5 (0.1 = 24 pts on a 24,000 index)")
        if cfg["exit_basis"] == "spot" and cfg[f"{k}_mode"] == "abs" and cfg[f"{k}_value"] < 1:
            return _abort(cfg, strategy_id,
                          f"spot basis: {k}_value {cfg[f'{k}_value']} points is below 1 — "
                          f"did you mean pct?")
    ...
    diag = {
        ...
        "strike_fallbacks": 0,
        "exit_basis": cfg["exit_basis"], "eff_sl_sum": 0.0, "eff_tp_sum": 0.0,   # ── STFC_SPOT_20260913 ──
    ...
    for i, day in enumerate(days):
        ...
        for s in sigs:
            ...
            sl_prem = tp_prem = sl_spot = tp_spot = None
            if spot_basis:
                # ── STFC_SPOT_20260913 ── levels on the INDEX; CE is long
                # spot, PE is short spot.
                sl_spot = spot_level(entry_spot, s.side, cfg["sl_mode"], cfg["sl_value"], is_stop=True)
                tp_spot = spot_level(entry_spot, s.side, cfg["tp_mode"], cfg["tp_value"], is_stop=False)
                if sl_spot is not None:
                    diag["eff_sl_sum"] += abs(entry_spot - sl_spot)
                if tp_spot is not None:
                    diag["eff_tp_sum"] += abs(tp_spot - entry_spot)
            else:
            ...
            _b = "s" if spot_basis else ""
            sl_tag = ("SLoff" if (sl_prem is None and sl_spot is None) else
                      f"{_b}SL{'P' if cfg['sl_mode'] == 'pct' else 'A'}{cfg['sl_value']:g}")
            tp_tag = ("TPoff" if (tp_prem is None and tp_spot is None) else
                      f"{_b}TP{'P' if cfg['tp_mode'] == 'pct' else 'A'}{cfg['tp_value']:g}")
            if spot_basis and cfg["spot_sl_trigger"] == "close":
                sl_tag += "@close"
            ...
            t = STFCTrade(
                ...
                sl=(round(sl_spot, 2) if spot_basis else round(sl_prem, 2)) if (sl_spot if spot_basis else sl_prem) is not None else None,
                tp=(round(tp_spot, 2) if spot_basis else round(tp_prem, 2)) if (tp_spot if spot_basis else tp_prem) is not None else None,
            ...
            for m in range(entry_min, eod_min + 1):
                ...
                if o is not None:
                    ...
                    diag["stale_marks"] += 1
                if spot_basis:
                    # ── STFC_SPOT_20260913 ── spot-level exits; the option
                    # fill is a CONVENTION (no premium print of its own).
                    b = spot_by_min.get(m)
                    if b is not None:
                        hit = None
                        if sl_spot is not None and spot_breached(
                                s.side, sl_spot, b, is_stop=True, trigger=cfg["spot_sl_trigger"]):
                            hit = "SL"
                        elif tp_spot is not None and spot_breached(
                                s.side, tp_spot, b, is_stop=False, trigger=cfg["spot_sl_trigger"]):
                            hit = "TP"
                        if hit is not None:
                            if cfg["spot_sl_trigger"] == "close":
                                m2 = min(m + 1, eod_min)
                                o2 = ob.get(ds + m2 * 60)
                                px = (float(o2.open) if (o2 is not None and o2.open)
                                      else float(o.close) if o is not None else pos["last_mark"])
                                close_trade(pos, ds + m2 * 60, px, hit)
                                exit_min = m2
                                break
                            if o is None:
                                px = pos["last_mark"]
                            elif hit == "SL":
                                px = float(o.close) if cfg["spot_sl_fill"] == "close" else float(o.low)
                            else:
                                px = (float(o.high) if cfg["spot_tp_fill"] == "high"
                                      else float(o.low) if cfg["spot_tp_fill"] == "low"
                                      else float(o.close))
                            close_trade(pos, ds + m * 60, px, hit)
                            exit_min = m
                            break
                elif o is not None:
    ...
    if diag["entries"]:
        ...
        diag["eff_sl_avg_pts"] = round(diag["eff_sl_sum"] / diag["entries"], 2)   # ── STFC_SPOT_20260913 ──
        diag["eff_tp_avg_pts"] = round(diag["eff_tp_sum"] / diag["entries"], 2)
