# backend/app/backtest/pst/backtest_pst_runner.py
#
# ── PST_V1 RUNNER ── Pivot/SMA/SuperTrend spot-signal option scalper.
# All decision logic lives in pst_v1_engine + pst_indicators (pure, tested);
# this file is plumbing: SPOT + option corpus access, per-signal option
# selection, charges, PSTTrade rows for persist_run, DIAG, progress/cancel.
# Caller persists (routes / queue_worker), same as IC/HA/WICK.
#
# FILL CONVENTIONS (locked 2026-07-06, no-lookahead pair):
#   * SELECTION at a signal (ts = 3m bar completion): premium < cap,
#     nearest-below, priced off the last COMPLETED 1m option candle — the
...
#   * MONITORING starts at ts+60 — the fill candle's own extremes cannot
#     stop out a fill taken at its close.
#   * spot anchor for the point-targets = the SIGNAL BAR's close (engine).
#   * Both legs fill on the SAME symbol; leg identity in persisted rows =
#     qty (L1 = 2×lots vs L2 = 1×lots; direction always BUY). The spot
#     target itself is not persisted (tp column stays NULL — it is a SPOT
#     level, not an option price); exit_reason SPOT_TG carries the story.
#   * Pivots need the PREVIOUS session's spot H/L/C → the first tradable
...
try:
    from app.backtest.pst.pst_v1_engine import build_signals, run_day
    from app.backtest.ic.ic_v1_engine import select_strike
except ImportError:  # standalone tests
    from pst_v1_engine import build_signals, run_day  # type: ignore
...
class PSTTrade:
    ...
    entry_price: float
    sl: Optional[float]
    tp: Optional[float]           # NULL — spot targets are not option prices
    exit_ts: Optional[int]
    exit_price: Optional[float]
    exit_reason: Optional[str]    # SL | SPOT_TG | EOD
    qty: int
...
def _summarize(trades: List[PSTTrade], diag: dict) -> dict:
...
def run_pst_backtest(
    *,
    db_path: str,
    strategy_id: str,           # "PST_V1"
...
def _impl(*, db_path, strategy_id, underlying, date_from, date_to,
    ...
    if not legs:
        return {"run_id": None, "aborted": True,
                "reason": "PST_V1 needs at least one leg with lots > 0",
    ...
    for di, d in enumerate(spot_days, start=1):
        ...
        day = run_day(sig_res["signals"], legs, select_option, spot, eod_ts,
        ...
        for lt in day["trades"]:
            ...
            gross = (float(lt["exit_price"]) - float(lt["entry_price"])) * qty
            charges = 0.0
            if charges_for_long_trade is not None:
                try:
                    cr = charges_for_long_trade(
    ...
    write_audit_log(
        f"[BACKTEST][PST_V1] {underlying} {date_from}→{date_to}: "
