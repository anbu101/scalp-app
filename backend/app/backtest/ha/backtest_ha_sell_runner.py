# backend/app/backtest/ha/backtest_ha_sell_runner.py
#
# HA_SELL backtest runner. Option SELLING (SHORT) on NIFTY weekly options,
# 1-MINUTE Heikin Ashi candles, indicators derived from the OPTION contract.
#
# This is HA_V1's signal, INVERTED to a SHORT: we SELL the same selected
# contract the HA signal fires on, and buy it back to exit. Because we're
# short, the HA_V1 SL/TP levels SWAP ROLES:
#   seller TP = HA_V1 SL level (below entry) — price FALLS here → we profit.
#   seller SL = HA_V1 TP level (above entry) — price RISES here → we're stopped.
#
# EXIT (seller asymmetry, per spec):
#   TP → price falls to the TP level: WAIT for candle close, trigger on
#        1m CLOSE <= tp, book AT THE CLOSE (mirrors HA_V1 SL-on-close).
#   SL → price rises to the SL level: trigger INTRABAR on 1m HIGH >= sl,
#        book AT THE SL LEVEL exactly (mirrors HA_V1 TP-on-high).
#   Ambiguous (1m high>=sl AND 1m close<=tp same bar) → pessimistic SL (loss),
#        flagged.
#   EOD → buy back at the day's last close.
#
# P&L SHORT = (entry - exit) * qty. Charges via charges_for_SHORT_trade
# (STT on the ENTRY/sell leg).
#
...
# Charges via charges_for_short_trade (STT on the exit/sell leg).
...
class HATrade:
    ...
    direction: str = "SHORT"        # HA_SELL is always SHORT (option selling)
...
# Runner
# ----------------------------------------------------------------------
def run_ha_sell_backtest(
    *,
    db_path: str,
    strategy_id: str,           # "HA_SELL"
    ...
    with audit_muted():
        return _run_ha_sell_backtest_impl(
...
def _run_ha_sell_backtest_impl(
    *,
    db_path: str,
    strategy_id: str,           # "HA_SELL"
    underlying: str,            # "NIFTY"
    ...
    charges_for_short_trade = _resolve_charges_fn()
    ...
    # ── MIN_SL_GATE END ──
    # ── MAX_SL_CAP ── caps the SELLER'S stop distance (upside). 0 = disabled.
    ...
    # daily counters stay correct automatically. NOTE: filtering also preserves
    # the HA_V1 mirror property — run HA_V1 and HA_SELL with the SAME condition
    # subset to compare true opposites.
            ...
            for sym, b1 in items:
                ...
                if open_trade is not None and open_trade.symbol == sym:
                    ...
                    if (max_loss > 0 or max_profit > 0):
                        # SHORT open MTM = (entry - current) * qty (profit when price falls).
                        open_gross = (open_trade.entry_price - float(b1["close"])) * open_trade.qty
...
def _resolve_charges_fn():
    """Resolve charges_for_short_trade from whichever module path exists. Caches
    ...
    for path in _CHARGES_PATHS:
        try:
            ...
            fn = getattr(mod, "charges_for_short_trade", None)
            if fn is not None:
                _CHARGES_FN = fn
                try:
                    from app.event_bus.audit_logger import write_audit_log
                    write_audit_log(f"[BACKTEST][HA][CHARGES] using {path}.charges_for_short_trade")
                except Exception:
                    pass
                return _CHARGES_FN
        except Exception:
    ...
    try:
        from app.event_bus.audit_logger import write_audit_log
        write_audit_log(
            "[BACKTEST][HA][CHARGES][WARN] charges_for_short_trade NOT FOUND in any "
            f"of {_CHARGES_PATHS} — charges will be ZERO and net P&L will equal "
            "gross. Fix the import path in backtest_ha_sell_runner.py."
        )
    except Exception:
        pass
    _CHARGES_FN = None
...
def _close_trade(trade: HATrade, *, exit_ts: int, exit_price: float, reason: str, charges_fn) -> None:
    ...
    # SHORT gross = (entry - exit) * qty. charges_for_short_trade returns the
    # same short-signed gross and overwrites below; this keeps the no-charges
