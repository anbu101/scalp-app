...
DEFAULTS: dict = {
    ...
    "s2_enabled": False,               # ── BRK_V1_S2_20260831 ── second window in the day
    "s2_select_time": "10:25",         # ── BRK_V1_S2_20260831 ──
    "s2_entry_first": "10:30",         # ── BRK_V1_S2_20260831 ──
    "s2_entry_last": "10:30",          # ── BRK_V1_S2_20260831 ──
    "s2_only_if_flat": True,           # ── BRK_V1_S2_20260831 ── skip s2 minutes while s1 is open
    "s2_only_if_loss": False,          # ── BRK_V1_S2_20260831 ── skip s2 when s1 closed profitable
...
def _merge_cfg(override: Optional[dict]) -> dict:
    ...
    for k in ("s2_enabled", "s2_only_if_flat", "s2_only_if_loss"):   # ── BRK_V1_S2_20260831 ──
...
def _impl(*, db_path, strategy_id, underlying, date_from, date_to,
    ...
    # ── BRK_V1_S2_20260831 ── session-2 clock: after session 1, before EOD.
    s2_sel_min = _hhmm(cfg["s2_select_time"], 10 * 60 + 25)
    s2_first_min = _hhmm(cfg["s2_entry_first"], 10 * 60 + 30)
    s2_last_min = _hhmm(cfg["s2_entry_last"], 10 * 60 + 30)
    if cfg["s2_enabled"]:
        if not (last_min <= s2_sel_min and
                s2_sel_min + sustain <= s2_first_min <= s2_last_min < eod_min):
            return _abort(cfg, strategy_id,
                          (f"session-2 time order must be entry_last {cfg['entry_last']} "
                           f"<= s2_select {cfg['s2_select_time']} (+{sustain} sustain) "
                           f"<= s2_first {cfg['s2_entry_first']} <= s2_last "
                           f"{cfg['s2_entry_last']} < eod {cfg['eod_square_off']}"))
    ...
    diag = {
        ...
        "s2_entries": 0, "s2_ce_entries": 0, "s2_pe_entries": 0,   # ── BRK_V1_S2_20260831 ──
        "s2_days_no_candidate": 0, "s2_days_no_break": 0,
        "s2_days_blocked_open": 0, "s2_days_skipped_win": 0,
    ...
    for i, day in enumerate(days):
        ...
        # ── BRK_V1_S2_20260831 ── ONE exit ladder for both sessions. Returns the exit
        # minute (the trade is always closed by EOD or the truncated tape).
        def run_ladder(pos: dict) -> int:
            ob = bars(pos["symbol"])
            ...
            for m in range(pos["entry_min"], eod_min + 1):
                ...
                if cfg["trail_mode"] == "ratchet":
                    # ── BRK_V1_RATCHET_20260831 ── stop follows the highest high, only up.
        ...
        # ── BRK_V1_S2_20260831 ── session 1 ladder, then the optional second window.
        traded = False
        s1_pos = pos
        s1_exit_min: Optional[int] = None
        if s1_pos is not None:
            s1_exit_min = run_ladder(s1_pos)
            traded = True

        if cfg["s2_enabled"]:
            if (cfg["s2_only_if_loss"] and s1_pos is not None
                    and s1_pos["trade"].net_pnl > 0):
                diag["s2_days_skipped_win"] += 1
            else:
                # Fresh selection at the session-2 instant, full universe.
                s2_sel_ts = ds + (s2_sel_min - 1) * 60
                prints2: Dict[str, Dict[str, float]] = {"CE": {}, "PE": {}}
                for c in universe:
                    itype = c.get("instrument_type")
                    if itype not in ("CE", "PE"):
                        continue
                    b = bars(c["tradingsymbol"]).get(s2_sel_ts)
                    if b is None:
                        continue
                    prints2[itype][c["tradingsymbol"]] = float(b.close)
                    meta[c["tradingsymbol"]] = c
                ce2 = pick_candidate(prints2["CE"], below=cfg["select_below"],
                                     floor=cfg["select_min"])
                pe2 = pick_candidate(prints2["PE"], below=cfg["select_below"],
                                     floor=cfg["select_min"])
                if ce2 is None and pe2 is None:
                    diag["s2_days_no_candidate"] += 1
                else:
                    closes2 = {}
                    for side, sym in (("CE", ce2), ("PE", pe2)):
                        closes2[side] = ({(b.ts - ds) // 60: float(b.close)
                                          for b in bars(sym).values()} if sym else {})
                    s2_pos = None
                    s2_saw = False
                    blocked = False
                    for m in range(s2_first_min, s2_last_min + 1):
                        if (cfg["s2_only_if_flat"] and s1_pos is not None
                                and s1_exit_min is not None and s1_exit_min >= m):
                            blocked = True   # morning still open at this minute
                            continue
                        ce_ok = bool(ce2) and confirmed_at(
                            closes2["CE"], m, level=cfg["break_above"], sustain=sustain)
                        pe_ok = bool(pe2) and confirmed_at(
                            closes2["PE"], m, level=cfg["break_above"], sustain=sustain)
                        if not (ce_ok or pe_ok):
                            continue
                        s2_saw = True
                        ce_first = pe_first = None
                        if ce_ok and pe_ok:
                            ce_first = first_break_minute(closes2["CE"], from_min=s2_sel_min,
                                                          to_min=m - 1, level=cfg["break_above"])
                            pe_first = first_break_minute(closes2["PE"], from_min=s2_sel_min,
                                                          to_min=m - 1, level=cfg["break_above"])
                        side = choose_side(ce_ok=ce_ok, pe_ok=pe_ok,
                                           policy=cfg["both_policy"],
                                           ce_first=ce_first, pe_first=pe_first,
                                           ce_px=closes2["CE"].get(m - 1),
                                           pe_px=closes2["PE"].get(m - 1))
                        if side is None:
                            break
                        s2_pos = open_pos(ce2 if side == "CE" else pe2, side, m, "BRK·S2")
                        if s2_pos is None:
                            diag["days_no_fill"] += 1
                            continue
                        diag["s2_entries"] += 1
                        diag["s2_ce_entries" if side == "CE" else "s2_pe_entries"] += 1
                        break
                    if s2_pos is not None:
                        run_ladder(s2_pos)
                        traded = True
                    elif blocked and not s2_saw:
                        diag["s2_days_blocked_open"] += 1
                    elif not s2_saw:
                        diag["s2_days_no_break"] += 1
                    elif blocked:
                        diag["s2_days_blocked_open"] += 1
