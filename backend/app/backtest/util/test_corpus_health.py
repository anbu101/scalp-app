...
# ── bands: disjoint, with a real GAP between the two frames ──
for f in (2.0, 3.0, 5.0, 10.0):
    ck(f"bands disjoint at factor {f:g}", CH.bands_disjoint(f),
       str(CH.frame_band(f)))
_lo, _hi, _slo, _shi = CH.frame_band(2.0)
ck("there is a gap, not just an absence of overlap", _slo - _hi > 0.15,
   f"gap ({_hi}, {_slo})")
# the exact HDFCBANK residue ratios must land in that gap
for r in (1.416, 1.425, 1.431, 1.442):
    ck(f"observed bad ratio {r} classifies as junk",
       not (_lo <= r <= _hi) and not (_slo <= r <= _shi))
# and a real ladder ratio must NOT
for r in (0.90, 1.00, 1.13):
    ck(f"real ladder ratio {r} stays in frame", _lo <= r <= _hi)

# a row in the old overlap strip must now count as NATIVE, not scaled
with tempfile.TemporaryDirectory() as td:
    # prune operates on an ALREADY single-frame corpus, so repair it first
    db5 = str(Path(td) / "B.db")
    build(db5)
    CH.repair_frame_split(db5, "X", ex_date=EX.isoformat(), factor=2.0,
                          dry_run=False, backup=False)
    c5 = sqlite3.connect(db5)
    # post-repair the 2025-09 side still has spot 800; strike 1144 is ratio
    # 1.43 — the exact HDFCBANK residue shape. Under the OLD bands this was
    # reported as `scaled` on a corpus with only one frame.
    for k in range(3):
        c5.execute("INSERT INTO backtest_candles_1m VALUES "
                   "(90000?,?,'X','XSTRIP','CE',1144,'2025-09-30',2,2,2,2,0,0)"
                   .replace("90000?", str(90000 + k)),
                   (ts_for(date(2025, 9, 1), k),))
    c5.commit(); c5.close()
    cen5 = CH.frame_scan(db5, "X", factor=2.0)
    row = [r for r in cen5 if r["month"] == "2025-09"][0]
    ck("old overlap strip is now classified out-of-frame, not DUAL",
       row["scaled"] == 0 and row["junk"] == 3, str(row))

    # ── prune ──
    p5 = CH.prune_out_of_frame(db5, "X", factor=2.0, dry_run=True)
    ck("prune dry run finds the strip rows", p5["options_delete"] == 3,
       str(p5["options_delete"]))
    ck("prune dry run names the offenders", len(p5["sample"]) >= 1)
    ck("prune dry run writes nothing",
       sqlite3.connect(db5).execute(
           "SELECT count(*) FROM backtest_candles_1m WHERE "
           "tradingsymbol='XSTRIP'").fetchone()[0] == 3)
    CH.prune_out_of_frame(db5, "X", factor=2.0, dry_run=False, backup=False)
    c5 = sqlite3.connect(db5)
    ck("prune deletes them",
       c5.execute("SELECT count(*) FROM backtest_candles_1m WHERE "
                  "tradingsymbol='XSTRIP'").fetchone()[0] == 0)
    ck("prune is re-runnable (no doubling to undo)",
       CH.prune_out_of_frame(db5, "X", factor=2.0, dry_run=False,
                             backup=False)["options_delete"] == 0)
    c5.close()

    # prune must REFUSE to eat a whole frame — that is repair's job
    db6 = str(Path(td) / "C.db")
    build(db6)
    try:
        CH.prune_out_of_frame(db6, "X", factor=2.0, dry_run=True)
        ck("prune refuses a dual-frame corpus", False)
    except SystemExit:
        ck("prune refuses a dual-frame corpus", True)

    # ── frame-break guard ──
    db7 = str(Path(td) / "D.db")
    build(db7)
    ck("no break stamped -> no veto",
       CH.frame_break_reason(db7, date(2021, 1, 1), date(2026, 1, 1)) is None)
    CH.repair_frame_split(db7, "X", ex_date=EX.isoformat(), factor=2.0,
                          dry_run=False, backup=False)
    ck("range spanning the break is vetoed",
       CH.frame_break_reason(db7, date(2025, 8, 1), date(2025, 9, 30))
       is not None)
    ck("range ending ON the break day is vetoed (that day is post-frame)",
       CH.frame_break_reason(db7, date(2025, 8, 1), EX) is not None)
    ck("range ending the day BEFORE is allowed",
       CH.frame_break_reason(db7, date(2025, 8, 1),
                             EX - timedelta(days=1)) is None)
    ck("range starting ON the break day is allowed",
       CH.frame_break_reason(db7, EX, date(2026, 1, 1)) is None)
    ck("veto message names the date and the fix",
       EX.isoformat() in CH.frame_break_reason(
           db7, date(2025, 8, 1), date(2025, 9, 30)))
    ck("missing corpus never raises",
       CH.frame_break_reason(str(Path(td) / "nope.db"),
                             date(2025, 1, 1), date(2026, 1, 1)) is None)

# ── sanitizer watermark: repair/prune must never blind the sanitizer ──
def _stamp(db, watermark):
    c = sqlite3.connect(db)
    c.execute("""CREATE TABLE IF NOT EXISTS corpus_sanitizer_meta(
        id INTEGER PRIMARY KEY CHECK (id = 1), sanitizer_version INTEGER NOT NULL,
        rowid_watermark INTEGER NOT NULL, last_run_ts INTEGER NOT NULL,
        rows_quarantined_total INTEGER NOT NULL DEFAULT 0)""")
    c.execute("INSERT OR REPLACE INTO corpus_sanitizer_meta VALUES (1,1,?,0,0)",
              (watermark,))
    c.commit(); c.close()
...
def _stamp_rows(db):
    c = sqlite3.connect(db)
    try:
        return c.execute("SELECT count(*) FROM corpus_sanitizer_meta").fetchone()[0]
    finally:
        c.close()


with tempfile.TemporaryDirectory() as td:
    dbr = str(Path(td) / "R.db")
    build(dbr)
    _stamp(dbr, 10 ** 9)                       # watermark far above max rowid
    CH.repair_frame_split(dbr, "X", ex_date=EX.isoformat(), factor=2.0,
                          dry_run=False, backup=False)
    ck("repair clears the sanitizer stamp it would otherwise blind",
       _stamp_rows(dbr) == 0)

    dbp = str(Path(td) / "P.db")
    build(dbp)
    CH.repair_frame_split(dbp, "X", ex_date=EX.isoformat(), factor=2.0,
                          dry_run=False, backup=False)
    _stamp(dbp, 10 ** 9)
    CH.prune_out_of_frame(dbp, "X", factor=2.0, dry_run=False, backup=False)
    ck("prune clears it too", _stamp_rows(dbp) == 0)

    # dry runs must NOT touch the stamp
    dbd = str(Path(td) / "D2.db")
    build(dbd)
    _stamp(dbd, 10 ** 9)
    CH.repair_frame_split(dbd, "X", ex_date=EX.isoformat(), factor=2.0,
                          dry_run=True)
    ck("dry run leaves the stamp alone", _stamp_rows(dbd) == 1)

    # resync repairs a corpus damaged by an EARLIER repair (pre-fix)
    dbs = str(Path(td) / "S.db")
    build(dbs)
    _stamp(dbs, 10 ** 9)
    r = CH.resync_sanitizer(dbs)
    ck("resync detects a stale watermark", r["was_stale"] and r["cleared"])
    ck("resync actually cleared it", _stamp_rows(dbs) == 0)
    ck("resync is a no-op when the stamp is healthy",
       CH.resync_sanitizer(dbs)["was_stale"] is False)
    dbn = str(Path(td) / "N.db")
    build(dbn)
    ck("resync never raises when the sanitizer never ran",
       CH.resync_sanitizer(dbn)["cleared"] is False)
