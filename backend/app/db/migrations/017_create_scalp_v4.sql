-- =========================================================
-- 017_create_scalp_v4.sql
-- SAFE • IDEMPOTENT • NO DATA LOSS
-- =========================================================
-- Purpose:
--   SCALP_V4 is a TEST option-BUYING strategy derived from SCALP_V3.
--   It is byte-for-byte identical to SCALP_V3 EXCEPT for ONE extra
--   entry condition (EMA8_close must NOT be above EMA20_High), applied
--   as a veto in the V4 tick engine AFTER the shared condition engine
--   runs. The execution model is unchanged: the contract that fires the
--   signal (the "signal" instrument, e.g. 24500CE) is NEVER traded — it
--   is only tracked for its SL/TP. V4 BUYS the highest-premium
--   opposite-side option (the "hedge", e.g. 24450PE) and protects it
--   with an SL-only GTT at (hedge_fill - MAX_SL). The hedge is exited
--   when EITHER the signal contract hits its own SL/TP, OR the hedge's
--   own SL-only GTT fires.
--
--   Because ONE logical trade spans TWO different instruments in two
--   different premium spaces, V4 CANNOT reuse trades / paper_trades
--   (single-symbol schema). This migration creates ONE own table,
--   scalp_v4_trades, with a paper flag (0=live, 1=paper) — mirroring
--   the scalp_v3_trades single-table+paper-flag convention.
--
--   No other strategy (BB_V1, BB_V2, HA_V1, SCALP_V1, SCALP_V2,
--   SCALP_V3) reads or writes this table, so creation is fully
--   isolated. Removing V4 later = DROP this one table + delete the V4
--   engine package.
--
...
-- Global single-trade gate + manager scan: find the OPEN V4 trade fast.
