# backend/app/db/scalp_v3_repo.py
#
# SCALP_V3 — TEST option-BUYING hedge strategy (derived from SCALP_V1).
#
# OWNERSHIP / ISOLATION:
#   This repo is the ONLY module that reads/writes scalp_v3_trades.
#   It NEVER touches trades / paper_trades. No other strategy reads this
#   table. Deleting SCALP_V3 later = DROP scalp_v3_trades + delete this file.
#
...
#   1. insert_v3_trade()     — provisional hedge_entry (protected limit), OPEN.
...
#   table inline if missing. Inline DDL is kept identical to 016_create_scalp_v3.sql.
...
# Inline DDL MUST stay identical to 016_create_scalp_v3.sql.
# --------------------------------------------------
...
def _ensure_schema():
    ...
    try:
        ...
        if row is None:
            write_audit_log(
                "[DB][V3][SCHEMA_GUARD] scalp_v3_trades MISSING — creating inline "
                "(migration 016 may have been marked complete on partial failure)"
            )
        ...
        write_audit_log(f"[DB][V3][SCHEMA_GUARD][ERROR] {e}")
...
def insert_v3_trade(
    *,
    ...
    try:
        conn.execute(
            """
            INSERT INTO scalp_v3_trades (
                v3_trade_id, strategy_name, session_date, paper,
            ...
            VALUES (?, 'SCALP_V3', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'LONG',
                    ?, ?, ?, ?, 'OPEN', ?, ?, ?)
            """,
            (
                v3_trade_id, _session_date(), 1 if paper else 0,
                signal_symbol, signal_token, signal_side,
                signal_entry_price, signal_sl, signal_tp, signal_candle_ts,
        ...
        conn.commit()
        write_audit_log(
            f"[DB][V3] OPEN id={v3_trade_id} paper={int(paper)} "
...
def confirm_hedge_fill(
    *,
    ...
    try:
        ...
        cur = conn.execute(
            """
            UPDATE scalp_v3_trades
            SET hedge_entry_price = ?,
                hedge_sl          = ?,
                updated_at        = ?
            WHERE v3_trade_id = ? AND state = 'OPEN'
            """,
            (fill_price, new_sl, int(time.time()), v3_trade_id),
        )
        conn.commit()
        if cur.rowcount == 0:
            write_audit_log(
                f"[DB][V3][SKIP] FILL_CONFIRM IGNORED id={v3_trade_id} "
                f"(row not OPEN)"
            )
        else:
            write_audit_log(
                f"[DB][V3] FILL_CONFIRMED id={v3_trade_id} "
                f"entry={fill_price} sl={new_sl} (=fill-{max_sl_points})"
            )
        return new_sl
    except Exception as e:
        conn.rollback()
        write_audit_log(f"[DB][V3][ERROR] FILL_CONFIRM FAILED id={v3_trade_id} ERR={e}")
        raise
...
def link_hedge_gtt(*, v3_trade_id: str, gtt_id: str):
    _ensure_schema()
    conn = get_conn()
    try:
        cur = conn.execute(
            ...
            WHERE v3_trade_id = ? AND state = 'OPEN'
            """,
            (gtt_id, int(time.time()), v3_trade_id),
        )
        conn.commit()
        if cur.rowcount == 0:
            write_audit_log(
                f"[DB][V3][SKIP] GTT_LINK IGNORED id={v3_trade_id} (row not OPEN)"
            )
        else:
            write_audit_log(f"[DB][V3] GTT LINKED id={v3_trade_id} gtt_id={gtt_id}")
    except Exception as e:
        conn.rollback()
        write_audit_log(f"[DB][V3][ERROR] GTT_LINK FAILED id={v3_trade_id} ERR={e}")
        raise
...
def close_v3_trade(
    *,
    ...
    try:
        ...
        if not row:
            diag = conn.execute(
                "SELECT state, exit_price FROM scalp_v3_trades WHERE v3_trade_id = ?",
                (v3_trade_id,),
            ).fetchone()
            if diag is None:
                write_audit_log(
                    f"[DB][V3][SKIP] CLOSE IGNORED id={v3_trade_id} "
        ...
        conn.execute(
            ...
            (
                ...
                realized, int(time.time()), v3_trade_id,
            ),
        )
        conn.commit()
        write_audit_log(
...
def get_open_v3_trade(*, paper: Optional[bool] = None):
    """
    Returns the one OPEN V3 trade as a dict, or None.
    ...
    try:
        if paper is None:
            cur = conn.execute(
                "SELECT * FROM scalp_v3_trades WHERE state = 'OPEN' LIMIT 1"
            )
        else:
            cur = conn.execute(
                "SELECT * FROM scalp_v3_trades WHERE state = 'OPEN' AND paper = ? LIMIT 1",
                (1 if paper else 0,),
            )
        r = cur.fetchone()
        if not r:
        ...
        write_audit_log(f"[DB][V3][ERROR] GET_OPEN FAILED ERR={e}")
        return None


def get_open_v3_trade_by_signal_token(signal_token: int):
    """Watcher map: resolve an incoming signal-token tick to its open V3 trade."""
    _ensure_schema()
    conn = get_conn()
    try:
        cur = conn.execute(
            "SELECT * FROM scalp_v3_trades "
        ...
        write_audit_log(f"[DB][V3][ERROR] GET_BY_SIGNAL_TOKEN FAILED ERR={e}")
        return None


def get_v3_trade_by_id(v3_trade_id: str):
    _ensure_schema()
    conn = get_conn()
    try:
        cur = conn.execute(
            "SELECT * FROM scalp_v3_trades WHERE v3_trade_id = ?",
            (v3_trade_id,),
        )
        r = cur.fetchone()
        if not r:
        ...
        write_audit_log(f"[DB][V3][ERROR] GET_BY_ID FAILED id={v3_trade_id} ERR={e}")
        return None


def get_all_open_v3_trades(*, paper: Optional[bool] = None):
    """EOD square-off / reconcile: all OPEN rows (should be ≤1 with global gate)."""
    _ensure_schema()
    conn = get_conn()
    try:
        if paper is None:
            cur = conn.execute("SELECT * FROM scalp_v3_trades WHERE state = 'OPEN'")
        else:
            cur = conn.execute(
                "SELECT * FROM scalp_v3_trades WHERE state = 'OPEN' AND paper = ?",
                (1 if paper else 0,),
            )
        cols = [c[0] for c in cur.description]
        return [dict(zip(cols, r)) for r in cur.fetchall()]
    except Exception as e:
        write_audit_log(f"[DB][V3][ERROR] GET_ALL_OPEN FAILED ERR={e}")
        return []


def get_total_pnl_v3(*, paper: Optional[bool] = None) -> float:
    """Realized P&L across CLOSED V3 trades (LONG hedge)."""
    _ensure_schema()
    conn = get_conn()
    try:
        if paper is None:
            rows = conn.execute(
                "SELECT realized_pnl FROM scalp_v3_trades "
        ...
        write_audit_log(f"[DB][V3][ERROR] PNL_FETCH FAILED ERR={e}")
        return 0.0
...
#
# Mirrors get_total_pnl_v3's filters (state=CLOSED, realized_pnl IS NOT NULL so
...
# ((exit - entry) * qty) — V3 records no charges, so the summary is gross.
# ==================================================

def get_closed_paper_v3_trades_today() -> list:
    """
    Return today's CLOSED PAPER V3 trades as dicts with the fields the EOD
...
def reconcile_stale_open_v3_trades(*, dry_run: bool = True) -> int:
    _ensure_schema()
    conn = get_conn()
    rows = conn.execute(
        "SELECT v3_trade_id, signal_symbol, hedge_symbol, hedge_entry_price, "
        "state, entry_time FROM scalp_v3_trades WHERE state = 'OPEN' "
        "ORDER BY entry_time DESC"
    ).fetchall()

    if not rows:
        write_audit_log("[RECONCILE][V3][STALE] no stale OPEN trades at startup")
        return 0

    for r in rows:
        write_audit_log(
            f"[RECONCILE][V3][STALE] STALE_OPEN id={r['v3_trade_id']} "
    ...
    if dry_run:
        write_audit_log(
            f"[RECONCILE][V3][STALE] dry_run=True, {len(rows)} row(s) "
    ...
    for r in rows:
        try:
            ...
            write_audit_log(
                f"[RECONCILE][V3][STALE][ERROR] id={r['v3_trade_id']} ERR={e}"
            )
