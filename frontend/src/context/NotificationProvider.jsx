...
import { applyTheme, normalizeTheme, currentTheme } from "../theme";   // ── THEME_PHASE1_20260831 ── ── THEME_THALAPATHY_20260906 ── currentTheme
...
const AudioAlerts = {
  ...
  // ── THEME_THALAPATHY_20260906 ── target-hit dispatcher + synthesised whistle. Whistle fires only
  // when the Thalapathy theme is active; every other theme keeps the stock TP tone.
  // Same discipline as _playSequence: one fresh AudioContext, closed after use.
  targetHit() {
    if (currentTheme() === "thalapathy") this._playWhistle();
    else this.takeProfitHit();
  },
  // Two rising sweeps ("wheet-wheet"), sine carrier with a light vibrato, quick
  // attack, exponential decay. No audio file — nothing to bundle or license.
  _playWhistle() {
    try {
      const Ctx = window.AudioContext || window.webkitAudioContext;
      if (!Ctx) return;
      const c = new Ctx();
      const schedule = () => {
        const sweeps = [[0.00, 1750, 2650, 0.24], [0.32, 1750, 2750, 0.30]];
        let end = 0;
        for (const [at, f0, f1, dur] of sweeps) {
          ...
          try {
            ...
            osc.frequency.exponentialRampToValueAtTime(f1 * 0.92, t + dur);
            vib.type = "sine"; vib.frequency.value = 28; vibGain.gain.value = 40;
            vib.connect(vibGain); vibGain.connect(osc.frequency);
            gain.gain.setValueAtTime(0.0001, t);
            gain.gain.exponentialRampToValueAtTime(0.28, t + 0.02);
            gain.gain.setValueAtTime(0.28, t + dur * 0.6);
            gain.gain.exponentialRampToValueAtTime(0.0001, t + dur);
            osc.connect(gain); gain.connect(c.destination);
            osc.start(t); vib.start(t);
            osc.stop(t + dur + 0.02); vib.stop(t + dur + 0.02);
          } catch { /* skip this sweep */ }
          end = Math.max(end, at + dur);
        }
        setTimeout(() => { try { c.close(); } catch {} }, (end + 0.15) * 1000);
      };
      if (c.state === "suspended") {
        c.resume().then(schedule).catch(() => { try { c.close(); } catch {} });
      } else {
...
export function NotificationProvider({ children, health }) {
  ...
  const fireEvent = useCallback((evt) => {
    ...
    if (s.notify_audio && perRuleAllows) {
      ...
      else if (type === "TP") AudioAlerts.targetHit();   // ── THEME_THALAPATHY_20260906 ── whistle in Thalapathy, stock tone elsewhere
