// frontend/src/strategies/tma2/TMA2Panel.jsx
//
// ── TMA_V2 dashboard panel ── (TMAPanel conventions, 2026-08-19)
// Four-EMA STACK (13/55/89/144 @5m spot) credit spread: SHORT the
// trend-opposite side + far-OTM hedge. ONE position at a time in EITHER
// direction (E1 bearish stack / E2 bullish stack share a single slot).
// Live is SELL-spread only; the crossover exit is always ON at EMA13×EMA55. Open-group card shows the SELL leg's progress bars
...
export default function TMA2Panel({ ltpMap = {} }) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        const [tr, st] = await Promise.all([
          fetch(`${getApiBase()}/api/tma2/trades?limit=100`).then((r) => r.json()),
          fetch(`${getApiBase()}/api/tma2/status`).then((r) => r.json()),
        ]);
    ...
    async function cfg() {
      try {
        const r = await fetch(`${getApiBase()}/api/strategy-config/TMA_V2`);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: spacing.lg }}>
      <div style={{ ...card, borderLeft: `3px solid ${ACCENT}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, color: colors.text.primary }}>
            {showParams ? "TMA V2" : stratName("TMA_V2", false)}   {/* ── UI_MASK ── */}
            ...
            <span style={{ marginLeft: 8, verticalAlign: "middle" }}>
              <BrokerChip strategyId="TMA_V2" /> {/* ACC2_W3 */}
            </span>
          </div>
          <div style={{ fontSize: 11, color: colors.text.muted, marginTop: 3 }}>
            {/* ── UI_MASK ── the admin subtitle is the full recipe */}
            {showParams
              ? "EMA 13/55/89/144 STACK @5m spot · credit spread — SELLS opposite the trend + far-OTM hedge · SL/TP/XOVER(13×55) on the sold premium · one position at a time"
      ...
      {status.disabled && (
        <div style={{ ...card, color: colors.loss, fontSize: 12, fontWeight: 700 }}>
          ⚠ TMA2 manager DISABLED (boot reconciliation mismatch) — resolve manually and restart the app.
        </div>
      )}
      {sigEng.frozen && (
        <div style={{ ...card, color: colors.loss, fontSize: 12, fontWeight: 700 }}>
          {showParams
            ? <>⚠ Signal engine FROZEN ({sigEng.freeze_reason || "prefix instability"}) — no new TMA2 entries today.</>
