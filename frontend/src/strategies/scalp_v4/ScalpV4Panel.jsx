/**
 * SCALP V4 PANEL  (option-BUYING hedge — TEST strategy)
 *
 * Intended path: src/strategies/scalp_v4/ScalpV4Panel.jsx
 *
 * SCALP_V4 is fundamentally different from V1/V2 and needs its own visual story:
 *   - The contract that FIRES the signal (e.g. 24500CE) is TRACKED, never traded.
 *   - V4 BUYS the highest-premium OPPOSITE-side option (e.g. 24450PE) — the hedge.
 ...
 *
 * Data: getScalpV4State() → { mode, selection:{CE,PE}, open_trade } and the
...
import { getScalpV4State, getStrategyConfig } from "../../api";
import { EmptyState } from "../../components/LoadingStates";
import { colors, spacing } from "../../tokens";

const STRATEGY_ID = "SCALP_V4";
...
const C = {
  ...
  // SCALP_V4 accent — orange (option-buying identity; distinct from V1 amber, V2 violet, V3 pink)
  v4:        "#f97316",
  v4Dim:     "rgba(249,115,22,0.13)",
};
...
/* ─── Hero trade card — BOTH legs of the open V4 trade ─── */
function HeroTradeCard({ trade, ltpMap }) {
  ...
  return (
    <div style={{
      background: C.bgCard,
      border: `1px solid ${C.v4}`,
      borderTop: `3px solid ${C.v4}`,
      borderRadius: 8, padding: spacing.md,
      display: "flex", flexDirection: "column", gap: spacing.sm,
    }}>
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
        <span style={{ fontSize: 11, fontWeight: 800, color: C.v4, letterSpacing: "0.5px", textTransform: "uppercase" }}>
          ● Active Hedge Trade
        </span>
      ...
      <div style={{ display: "flex", gap: spacing.md, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 200, background: C.bgCard, borderRadius: 6, padding: spacing.sm,
          border: `1px solid ${C.v4}40` }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: C.v4, textTransform: "uppercase", letterSpacing: "0.5px" }}>
              🛒 Bought (hedge)
            </span>
  ...
  return (
    <div onClick={onBecomePrimary} style={{
      ...
      <div style={{ writingMode: "vertical-rl", textOrientation: "mixed", transform: "rotate(180deg)",
        fontSize: 11, fontWeight: 800, color: C.v4, letterSpacing: "1.5px", textTransform: "uppercase" }}>
        SCALP V4
      </div>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
        <span style={{ width: 10, height: 10, borderRadius: "50%",
          background: inTrade ? C.v4 : C.textMuted,
          boxShadow: inTrade ? `0 0 8px ${C.v4}` : "none" }} />
        <span style={{ fontSize: 8, color: inTrade ? C.v4 : C.textMuted, fontWeight: 700 }}>
          {inTrade ? "LIVE" : "ARM"}
        </span>
      </div>
      {inTrade && (
...
/* ─── Main component ─── */
export default function ScalpV4Panel({ ltpMap, isPrimary, onBecomePrimary }) {
  ...
  useEffect(() => {
    async function loadFast() {
      try { setState(await getScalpV4State()); } catch {}
    }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: spacing.md, padding: "10px 14px",
        ...
        <div style={{ fontSize: 12, fontWeight: 800, color: C.v4, letterSpacing: "1px", textTransform: "uppercase" }}>
          SCALP V4
        </div>
        ...
        <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 9px", borderRadius: 4,
          background: inTrade ? C.v4Dim : C.bgSurf, color: inTrade ? C.v4 : C.textMuted,
          border: `1px solid ${inTrade ? C.v4 : C.borderDim}`, textTransform: "uppercase" }}>
        ...
        <div>
          ...
          {rows.length === 0 ? (
            <EmptyState icon="📊" title="No strikes selected"
              description="Selected CE/PE strikes will appear here once V4 picks them." />
          ) : (
