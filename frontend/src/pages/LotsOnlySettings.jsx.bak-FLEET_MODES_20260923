...
import { AppearanceSection } from "../components/AppSettingsSection";   // ── THEME_NONADMIN_20260908 ── theme picker for non-admin
...
const ORDERED_IDS = [
  ...
  "BB_V1", "BB_V2", "HA_V1", "TMA_V1", "TMA_V2",
  "VET_V1",
  "ORB_V1",   // ── ORB_V1 ──
...
// ── THEME_NONADMIN_20260908 ── "Appearance" rail item. Mirrors RailItem's chrome without any
...
export default function LotsOnlySettings() {
  ...
  // ── THEME_NONADMIN_20260908 ── "APPEARANCE" is a pseudo rail id (not a strategy); it is valid
  // even when the license has no strategies, so it is checked before ids[].
  const active = primaryId === "APPEARANCE" ? "APPEARANCE"
    : (primaryId && ids.includes(primaryId) ? primaryId : ids[0]);

  if (!ids.length) {
    // ── THEME_NONADMIN_20260908 ── no strategies: still let them pick a theme.
    return (
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* ── THEME_NONADMIN_20260908 ── pinned below strategies, mirrors the admin APP rail item */}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {active === "APPEARANCE"
          ? <AppearanceSection />   /* ── THEME_NONADMIN_20260908 ── */
