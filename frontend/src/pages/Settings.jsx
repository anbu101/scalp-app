...
// ── BRK_V1 BEGIN ── sealed Config A defaults (bible 2026-09-02). The
// sealed signal keys (select/break 180, window, SL 16 / TP 46, EOD 15:15)
// are backend-frozen and NOT rendered — only mode, lots and the Config B
// second-session switch live here. Mirrors the strategy_loader default.
const DEFAULT_BRK_CONFIG = {
  trade_execution_mode: "PAPER",
  s2_enabled: false,
  s2_only_if_flat: true,
  s2_only_if_loss: true,
  quantity: { lots: 1, lot_size: 65 },
};
// ── BRK_V1 END ──
...
function AdminSettings() {
  ...
  // ── VET_V1 END ──
  // ── BRK_V1 BEGIN ──
  const [brkConfig, setBrkConfig] = useState(null);
  const [brkStatus, setBrkStatus] = useState("");
  ...
  async function saveORB() {
    // ── ORB_SAVESTATUS_20260905 ── the status pill speaks TOKENS
    // ("success"/"error"), not sentences — "Saved." rendered as ✗ Failed
    // on a save that had actually persisted (found 2026-09-05).
    setOrbSaving(true);
    try {
      await saveStrategyConfig("ORB_V1", orbConfig);
      setOrbStatus("success"); setTimeout(() => setOrbStatus(""), 3000);
    } catch {
      setOrbStatus("error");   setTimeout(() => setOrbStatus(""), 3000);
    } finally { setOrbSaving(false); }
  }
  // ── ORB_V1 END ──
  ...
  // ── VET_V1 END ──
  // ── BRK_V1 BEGIN ── load / update / save. Saved payload merged OVER
  // defaults (incl. nested quantity) so backend-only sealed keys survive
  // every save untouched.
  async function loadBRK() {
    try {
      const d = await getStrategyConfig("BRK_V1");
      setBrkConfig({
        ...DEFAULT_BRK_CONFIG, ...d,
        quantity: { ...DEFAULT_BRK_CONFIG.quantity, ...(d?.quantity || {}) },
      });
    } catch { setBrkConfig({ ...DEFAULT_BRK_CONFIG }); }
  }
  function updateBRK(path, value) {
    const u = structuredClone(brkConfig);
    path.reduce((o, k, i) => { if (i === path.length - 1) o[k] = value; return o[k]; }, u);
    setBrkConfig(u);
  }
  async function saveBRK() {
    setBrkSaving(true);
    try {
      await saveStrategyConfig("BRK_V1", brkConfig);
      setBrkStatus("success"); setTimeout(() => setBrkStatus(""), 3000);
    } catch {
      setBrkStatus("error");  setTimeout(() => setBrkStatus(""), 3000);
    } finally { setBrkSaving(false); }
  }
  // ── BRK_V1 END ──
  ...
  function renderDetailBody(id) {
    switch (id) {
      ...
      case "BRK_V1": return (<>
              {/* ── BRK_V1 BEGIN ── Sealed Config A; the only decisions a
                  user makes are mode, size and whether Config B's second
                  session runs. Frozen and NOT rendered: select 09:25 <₹180,
                  break ≥₹180 (09:29 close), buy 09:30, SL −16 / TP +46,
                  EOD 15:15 — changing those leaves the seal. */}
              <Group title="Execution">
                <Field label="Mode" helper="PAPER = simulated at 1m closes · LIVE = real orders; SL+TP ride as ONE OCO GTT at the broker, cancelled-verified before any engine exit.">
                  <ModeToggle value={brkConfig.trade_execution_mode}
                    onChange={(v) => updateBRK(["trade_execution_mode"], v)} />
                </Field>
                <Field label="Lots" helper="65 qty per lot. Sealed evidence is per-lot linear: net ₹1.20L / DD ₹28.8K per lot over 2020-08..2026-08.">
              ...
              <Group title="Second session (Config B)">
                <Field label="Enable 10:25 window" helper="Config B: a second selection at 10:25, decision at 10:30 — same levels, same SL/TP. Backtest: +₹4.6L combined, BUT the slice is 2020/2024-weighted and NEGATIVE through 2026. Paper mandate: track the BRK·S2 slice separately; if it stays negative after a full paper quarter, switch this OFF.">
                  <input type="checkbox" checked={!!brkConfig.s2_enabled}
                    onChange={(e) => updateBRK(["s2_enabled"], e.target.checked)} />
                </Field>
                {brkConfig.s2_enabled && (<>
                  <Field label="Only if flat" helper="Skip the 10:30 decision minute while the morning trade is still open — capital is never doubled. Part of the sealed Config B; leave ON.">
                    <input type="checkbox" checked={!!brkConfig.s2_only_if_flat}
                      onChange={(e) => updateBRK(["s2_only_if_flat"], e.target.checked)} />
                  </Field>
                  <Field label="Only after a losing morning" helper="The second shot only redeploys after the morning LOST (or never traded). A profitable morning ends the day. Part of the sealed Config B; leave ON.">
                    <input type="checkbox" checked={!!brkConfig.s2_only_if_loss}
                      onChange={(e) => updateBRK(["s2_only_if_loss"], e.target.checked)} />
                  </Field>
                </>)}
              </Group>
              <Group title="Sealed (read-only)">
                <div style={{ fontSize: 12, color: colors.text.muted, lineHeight: 1.7 }}>
                  Select 09:25 nearest-below ₹180 (CE + PE) · buy the side whose 09:29 close holds ≥₹180, at 09:30 · SL −16 / TP +46 premium pts · EOD 15:15 · one trade per session · expected weekly only.
                  <div style={{ marginTop: 6 }}>Evidence, falsified levers and the paper mandate: <i>BRK_V1_Strategy_Bible_20260902.pdf</i>.</div>
                </div>
              </Group>
              {/* ── BRK_V1 END ── */}
            </>);
  ...
  return (
    <div style={{
      ...
      background: colors.bg.page,   // ── THEME_KOLAM_P2_20260906 ──
