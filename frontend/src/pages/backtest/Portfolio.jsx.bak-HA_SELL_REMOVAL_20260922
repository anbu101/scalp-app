...
const STRAT_LABEL = {
  ...
  VET_V1: "VET",   // ── VET_V1 ──
  ORB_V1: "ORB",   // ── ORB_V1 ──
  CBO_V1: "CBO", BRK_V1: "BRK",   // ── PF_FAVOURITES_20260830 ──
};
...
const LAUNCHABLE = ["SCALP_V1", "SCALP_V3", "SCALP_V5", "HA_V1", "HA_SELL", "IC_V1", "IC_V2", "TMA_V1", "TMA_V2", "TSG_V1"];   // ── PST_REMOVAL_20260909 ── PST launch chips removed
...
export default function Portfolio({
  ...
  const [favOnly, setFavOnly] = useState(false);   // ── PF_FAVOURITES_20260830 ──
  ...
  // ── PF_FAVOURITES_20260830 ── favourites float to the top (stable within each group);
  ...
  // ── PF_FAVOURITES_20260830 ── star toggle: optimistic, same endpoint as Compare Runs
  ...
  return (
    <div ref={wrapRef}>
      ...
      <Card style={{ overflowX: "auto", marginBottom: spacing.lg }}>
        {/* ── PF_FAVOURITES_20260830 ── favourites toggle */}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", ...typography.bodyMedium }}>
          <thead style={{ background: c.bg.tertiary }}>
            <tr>
              {["", "★", "Strat", "Period", "Params", "Net", "Max DD", "Trades", ""].map((h, i) => (   /* ── PF_FAVOURITES_20260830 ── */
          ...
          <tbody>
              ...
              return (
                <tr key={r.run_id}
                  ...
                  <td style={{ padding: "8px 10px", fontSize: 11, color: c.text.secondary, maxWidth: 380,
                    overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={r.note ? `${params}\n\nNote: ${r.note}` : params}>
                    {params || "—"}
                    {/* ── PF_FAVOURITES_20260830 ── note from Compare Runs */}
            ...
            {!doneRuns.length && (
              <tr><td colSpan={9} style={{ padding: "32px 0", textAlign: "center", color: c.text.muted, fontSize: 13 }}>   {/* ── PF_FAVOURITES_20260830 ── */}
