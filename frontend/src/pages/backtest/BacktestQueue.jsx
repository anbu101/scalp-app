...
function paramLineBase(cfg) {
  ...
  // ── BRK_V1_UI_20260830 ── same shared formatter as RunComparison.
  ...
  if (cfg.vwap_filter?.enabled) { const _mb = Number(cfg.vwap_filter.min_below_pts) || 0; p.push(_mb < 0 ? `vwap+${-_mb}tol` : `vwap${_mb > 0 ? _mb : ""}`); }   // ── SCALP_V1_UI_TIDY_20260827 ──
  if (cfg.atm_skew_filter?.enabled) p.push(`skew${cfg.atm_skew_filter.invert ? "-inv" : ""}${cfg.atm_skew_filter.parity_adjust ? `-par${cfg.atm_skew_filter.carry_pts}` : ""}${Number(cfg.atm_skew_filter.min_diff_pts) || 0}`);   // ── SCALP_V1_ATM_SKEW_PARITY_20260826 ──
