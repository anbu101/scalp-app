...
const HA = "HA_V1", IC = "IC_V1", TMA = "TMA_V1", TMA2 = "TMA_V2", TSG = "TSG_V1", GC = "GC_V1", VAP = "VAP_V1", VET = "VET_V1", BRK = "BRK_V1", ORB = "ORB_V1", STFC = "STFC_V1", FVG = "FVG_V1";   // ── FVG_V1_20260916 ── ── STFC_OPT_20260913 ──   // ── BRK_V1_UI_20260830 ── ── ORB_PCT_20260903 ──
...
const AXES = [
  ...
  { key: "target_pts", label: "Fixed target pts", strategies: [HA],
  ...
  { key: "tp_hold", label: "TP hold candles", strategies: [HA],
    hint: "0, 1, 2", parse: _num,
    apply: (c, v) => { c.tp_hold_extra_candles = v; }, fmt: (v) => `hold ${v}` },
  { key: "max_trades_side", label: "Max trades/side", strategies: [HA],
    hint: "3, 5, 10, 50", parse: _num,
    apply: (c, v) => { c.max_trades_per_side = v; }, fmt: (v) => `cap ${v}` },
  { key: "entry_conds", label: "Entry conditions", strategies: [HA],
  ...
  // ── BRK_V1_UI_20260830 ── BRK_V1: the level, the wait window and the exits ARE
  ...
  // ── BRK_V1_RATCHET_20260831 ── ratchet gap (sets trail_mode=ratchet; 0 = lock mode / off)
  ...
  // ── BRK_V1_FALLBACK_20260831 ── fallback on/off (0 = off, >0 = on with that min gain; use 0.001 for "on, any move")
  ...
  // ── BRK_V1_S2_20260831 ── second-session select instant; entry window = select+5m.
  { key: "brk_s2_sel", label: "BRK session-2 select (HH:MM)", strategies: [BRK],
    hint: "10:25, 11:25, 12:25, 13:25", parse: _hm,
    apply: (c, v) => {
      c.s2_enabled = true; c.s2_select_time = v;
      const [h, mi] = v.split(":").map(Number); const t = h * 60 + mi + 5;
      const f = `${String(Math.floor(t / 60)).padStart(2, "0")}:${String(t % 60).padStart(2, "0")}`;
      c.s2_entry_first = f; c.s2_entry_last = f;
    }, fmt: (v) => `S2@${v}` },
  ...
  // ── ORB_PCT_20260903 ── ORB_V1: the pre-registered hill axes (sl / orb
  // window / eod / block) plus the worst-month-attack axes (min range, ATR
  // gate, band, target). Value semantics for orb_sl / orb_rng follow the
  // form's unit selects (pts vs %), so switch the unit on the Run form
  // BEFORE launching a sweep on those axes.
  { key: "orb_sl", label: "ORB SL value (unit = form's SL unit)", strategies: [ORB],
    hint: "10, 15, 20  ·  % mode: 0.06, 0.08, 0.1, 0.12", parse: _num,
    apply: (c, v) => { c.sl_points = Math.abs(v); }, fmt: (v) => `SL${Math.abs(v)}` },
  { key: "orb_min", label: "ORB window (minutes)", strategies: [ORB],
    hint: "10, 15, 20", parse: _num,
    apply: (c, v) => { c.orb_minutes = Math.abs(v); }, fmt: (v) => `${Math.abs(v)}m` },
  { key: "orb_eod", label: "ORB EOD square-off", strategies: [ORB],
    hint: "12:30, 13:00, 13:30", parse: _hm,
    apply: (c, v) => { c.eod_square_off = v; }, fmt: (v) => `eod ${v}` },
  { key: "orb_block", label: "ORB entry block (HH:MM)", strategies: [ORB],
    hint: "11:00, 11:30, 12:00", parse: _hm,
    apply: (c, v) => { c.entry_block_time = v; }, fmt: (v) => `blk ${v}` },
  { key: "orb_tgt", label: "ORB target value (abs ₹ or %)", strategies: [ORB],
    hint: "40, 60, 80", parse: _num,
    apply: (c, v) => { c.target_value = Math.abs(v); }, fmt: (v) => `tp${Math.abs(v)}` },
  // ── ORB_UICLEAN_20260903 ── orb_rng axis removed: min-range filter rejected 2026-09-03 (−7.3L for zero worst-month effect).
  // ── ORB_UICLEAN_20260903 ── orb_atr axis removed: ATR gate rejected 2026-09-03 (−28.7L; continuation is hurt by the wide-range gate).
  { key: "orb_band", label: "ORB premium band ₹ (lo-hi)", strategies: [ORB],
    hint: "150-200, 100-200, 120-180", parse: _band,
    apply: (c, v) => { c.premium_min = v[0]; c.premium_max = v[1]; }, fmt: (v) => `₹${v[0]}–${v[1]}` },
  ...
  // ── TMA2_DTE_LOTS_20260922 ── per-DTE lot multiplier. Sweep tokens are
  ...
  { key: "tma_mtm_cut", label: "EOD loss cut", strategies: [TMA],
    ...
    apply: (c, v) => { c.cut_neg_mtm_eod = v; },
    fmt: (v) => (v ? "CutLosers" : "CarryAll") },
  // ── STFC_OPT_20260913 ── STFC_V1 grid axes. sl/tp value semantics follow
  ...
  { key: "stfc_sl", label: "STFC SL value (unit = form's basis + SL mode)", strategies: [STFC],
    hint: "spot %: 0.05, 0.1, 0.15  ·  spot pts: 15, 25, 40  ·  premium %: 10, 20, 30  ·  0 = off", parse: _num,
    apply: (c, v) => { c.sl_value = Math.abs(v); if (Math.abs(v) === 0) c.sl_mode = "off"; }, fmt: (v) => `SL${Math.abs(v)}` },
  { key: "stfc_tp", label: "STFC TP value (unit = form's basis + TP mode)", strategies: [STFC],
    hint: "spot %: 0.2, 0.3, 0.5  ·  spot pts: 40, 60, 100  ·  premium %: 20, 30, 50  ·  0 = off", parse: _num,
  ...
  { key: "stfc_block", label: "STFC entry block (HH:MM)", strategies: [STFC],
    ...
    apply: (c, v) => { c.entry_block_time = v; }, fmt: (v) => `blk ${v}` },
  // ── FVG_V1_20260916 ── FVG_V1 grid axes (keys mirror the lab runner's
  // --grid names so a lab table and an app sweep line up 1:1).
  { key: "fvg_tf", label: "FVG signal tf (min)", strategies: [FVG],
    hint: "3, 5", parse: _num,
    apply: (c, v) => { c.tf = Math.max(1, Math.round(v)); }, fmt: (v) => `${Math.round(v)}m` },
  { key: "fvg_disp", label: "FVG displacement (×ATR)", strategies: [FVG],
    hint: "1.2, 1.5, 2.0", parse: _num,
    apply: (c, v) => { c.disp_atr = Math.abs(v); }, fmt: (v) => `disp${Math.abs(v)}` },
  { key: "fvg_gap", label: "FVG gap width (×ATR)", strategies: [FVG],
    hint: "0.1, 0.15, 0.25", parse: _num,
    apply: (c, v) => { c.gap_min_atr = Math.abs(v); }, fmt: (v) => `gap${Math.abs(v)}` },
  { key: "fvg_hold", label: "FVG hold max (min)", strategies: [FVG],
    hint: "45, 60, 90, 120", parse: _num,
    apply: (c, v) => { c.hold_max = Math.max(1, Math.round(v)); }, fmt: (v) => `hold${Math.round(v)}` },
  { key: "fvg_rr", label: "FVG target R (0 = off)", strategies: [FVG],
    hint: "0, 3, 8", parse: _num,
    apply: (c, v) => { if (Math.abs(v) === 0) { c.tp_mode = "off"; } else { c.tp_mode = "rr"; c.tp_rr = Math.abs(v); } }, fmt: (v) => (Math.abs(v) === 0 ? "TPoff" : `TP${Math.abs(v)}R`) },
  { key: "fvg_be", label: "FVG breakeven R (0 = off)", strategies: [FVG],
    hint: "0, 1", parse: _num,
    apply: (c, v) => { c.be_rr = Math.abs(v); }, fmt: (v) => (Math.abs(v) === 0 ? "BEoff" : `BE${Math.abs(v)}R`) },
  { key: "fvg_slmin", label: "FVG stop floor (pts)", strategies: [FVG],
    hint: "10, 15, 20", parse: _num,
    apply: (c, v) => { c.sl_min_pts = Math.abs(v); }, fmt: (v) => `floor${Math.abs(v)}` },
  { key: "fvg_from", label: "FVG entries from (HH:MM)", strategies: [FVG],
    hint: "09:30, 09:45", parse: _hm,
    apply: (c, v) => { c.entry_from = v; }, fmt: (v) => `from ${v}` },
  { key: "fvg_until", label: "FVG entries until (HH:MM)", strategies: [FVG],
    hint: "10:30, 11:00, 11:30", parse: _hm,
    apply: (c, v) => { c.entry_until = v; }, fmt: (v) => `until ${v}` },
  { key: "fvg_offset", label: "FVG strike offset (steps OTM)", strategies: [FVG],
    hint: "0, 1", parse: _num,
    apply: (c, v) => { c.strike_offset = Math.round(v); }, fmt: (v) => `ATM${v > 0 ? "+" : ""}${Math.round(v)}` },
  { key: "fvg_dtemin", label: "FVG DTE min", strategies: [FVG],
    hint: "0, 3", parse: _num,
    apply: (c, v) => { c.dte_min = Math.max(0, Math.round(v)); }, fmt: (v) => `dte≥${Math.round(v)}` },
  { key: "fvg_trigger", label: "FVG stop trigger", strategies: [FVG],
    hint: "close, touch", parse: (tok) => { const v = tok.trim().toLowerCase(); return ["close", "touch"].includes(v) ? { v } : { err: `"${tok}" must be close or touch` }; },
    apply: (c, v) => { c.sl_trigger = v; }, fmt: (v) => `trig ${v}` },
  { key: "fvg_dir", label: "FVG direction", strategies: [FVG],
    hint: "CE, PE, BOTH", parse: (tok) => { const v = tok.trim().toUpperCase(); return ["CE", "PE", "BOTH"].includes(v) ? { v } : { err: `"${tok}" must be CE, PE or BOTH` }; },
    apply: (c, v) => { c.direction = v; }, fmt: (v) => v },
  { key: "fvg_fib", label: "FVG fib gate", strategies: [FVG],
    hint: "OFF, ON", parse: (tok) => { const v = tok.trim().toUpperCase(); return ["OFF", "ON"].includes(v) ? { v: v === "ON" } : { err: `"${tok}" must be OFF or ON` }; },
    apply: (c, v) => { c.fib_gate = v; }, fmt: (v) => (v ? "fib" : "nofib") },
  { key: "fvg_ifvg", label: "FVG trade inverted gaps", strategies: [FVG],
    hint: "OFF, ON", parse: (tok) => { const v = tok.trim().toUpperCase(); return ["OFF", "ON"].includes(v) ? { v: v === "ON" } : { err: `"${tok}" must be OFF or ON` }; },
    apply: (c, v) => { c.trade_ifvg = v; }, fmt: (v) => (v ? "ifvg" : "noifvg") },
  { key: "fvg_skipexp", label: "FVG skip expiry day", strategies: [FVG],
    hint: "OFF, ON", parse: (tok) => { const v = tok.trim().toUpperCase(); return ["OFF", "ON"].includes(v) ? { v: v === "ON" } : { err: `"${tok}" must be OFF or ON` }; },
    apply: (c, v) => { c.skip_expiry_day = v; }, fmt: (v) => (v ? "noExp" : "allDays") },
  { key: "fvg_mode", label: "FVG entry mode (strategy / controls)", strategies: [FVG],
    hint: "fvg, matched, fixed", parse: (tok) => { const v = tok.trim().toLowerCase(); return ["fvg", "matched", "fixed"].includes(v) ? { v } : { err: `"${tok}" must be fvg, matched or fixed` }; },
    apply: (c, v) => { c.entry_mode = v; }, fmt: (v) => (v === "fvg" ? "strategy" : `ctrl:${v}`) },
];
/* ── SWEEP_AXES END ── */
...
export default function SweepBuilder({
  ...
  const { combos, errors, warnings } = useMemo(() => {
    ...
    if (usingRR && (strategyId === HA) && baseline.target_override?.enabled) {
  ...
  const inputStyle = {
    ...
    fontFamily: "var(--c-font-ui)",   // ── THEME_KOLAM_P2_20260906 ──
  };
