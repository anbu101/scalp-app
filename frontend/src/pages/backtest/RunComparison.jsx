...
const PARAM_DEFS = [
  ...
  { key: "vet_leg",      label: "VET leg",        get: (r) => (r.config?.trend_len != null && r.config?.range_len != null) ? (r.config.leg_action === "SELL" ? "SELL" : "BUY") : null },
  { key: "vet_hedge",    label: "VET hedge wing", get: (r) => (r.config?.trend_len != null && r.config?.range_len != null && r.config?.leg_action === "SELL") ? (r.config.hedge_enabled ? `≤₹${r.config.hedge_max_premium}` : "naked") : null },
  ...
  { key: "vet_atm_pct",  label: "VET offset (% spot)", get: (r) => (r.config?.trend_len != null && r.config?.range_len != null && Number(r.config?.atm_offset_pct) !== 0) ? `${r.config.atm_offset_pct}%` : null },
  { key: "vet_prem_pct", label: "VET premium band %", get: (r) => { const c = r.config || {}; if (c.trend_len == null || c.range_len == null) return null; const lo = Number(c.premium_pct_min) > 0 ? `≥${c.premium_pct_min}%` : null; const hi = Number(c.premium_pct_max) > 0 ? `≤${c.premium_pct_max}%` : null; return (lo || hi) ? [lo, hi].filter(Boolean).join(" ") : null; } },
  { key: "vet_max_dte",  label: "VET max DTE", get: (r) => (r.config?.trend_len != null && r.config?.range_len != null && Number(r.config?.max_entry_dte) > 0) ? r.config.max_entry_dte : null },
...
const STRAT_LABEL = { SCALP_V1: "V1", SCALP_V3: "V3", SCALP_V5: "V5", HA_V1: "HA", HA_SELL: "HAS", WICK_V1: "WICK", IC_V1: "IC", IC_V2: "IC2", PST_V1: "PST", PST_SELL: "PSTS", PST_HEDGE: "PSTH", TMA_V1: "TMA", TMA_V2: "TMA2", TSG_V1: "TSG", GC_V1: "GC", VAP_V1: "VAP", VET_V1: "VET", CBO_V1: "CBO", BRK_V1: "BRK", ORV_V1: "ORV", ORB_V1: "ORB" };   // ── BRK_V1_UI_20260830 ── ── ORB_PCT_20260903 ──
...
function paramSummaryBase(run) {
  ...
  // ── ORB_V1_UI_20260903 ── MUST run before the CBO check: ORB configs also
  // carry both_side_policy + breakout_buffer_pts. Key: orb_minutes +
  // both_side_policy (ORV lacks the policy, CBO lacks orb_minutes).
  if (cfg.disp_atr != null && cfg.gap_min_atr != null) {   // ── FVG_V1_20260916 ── key: disp_atr + gap_min_atr
    return fvgParamSummary(cfg);
  }
  if (cfg.st_len != null && cfg.st_mult != null) {   // ── STFC_OPT_20260913 ── key: st_len + st_mult
  ...
  // ── BRK_V1_UI_20260830 ── BRK configs use their own field names.
...
export default function RunComparison({
  ...
  const [fFavOnly, setFFavOnly] = useState(false);   // ── RUN_FAVOURITES_20260830 ──
  ...
  // ── RUN_FAVOURITES_20260830 ── favourite star / note. Optimistic local update, then
  ...
  const filtered = useMemo(() => {
    ...
    if (fFavOnly) rows = rows.filter((r) => !!r.favourite);   // ── RUN_FAVOURITES_20260830 ──
    ...
    if (fSearch.trim()) {
      ...
      rows = rows.filter((r) =>
        ...
        (r.note || "").toLowerCase().includes(q)   // ── RUN_FAVOURITES_20260830 ── notes are searchable
      );
    }
    const getSort = (r) => {
      switch (sortKey) {
        case "favourite":   return r.favourite ? 1 : 0;   // ── RUN_FAVOURITES_20260830 ──
    ...
    return rows;
  }, [runs, fStrategy, fStatus, fProfitableOnly, fFavOnly, fSearch, sortKey, sortDir, colFilters, marginFor]);   // ── RUN_FAVOURITES_20260830 ──
    ...
    <Card elevated style={{ padding: spacing.lg, marginBottom: spacing.lg }}>
      <div style={{ display: "flex", gap: spacing.md, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          ...
          {["ALL", "SCALP_V1", "SCALP_V3", "SCALP_V5", "HA_V1", "IC_V1", "IC_V2", "TMA_V1", "TMA_V2", "TSG_V1", "GC_V1", "VET_V1", "CBO_V1", "BRK_V1", "ORB_V1" ].map((sId) /* ── ORB_UICLEAN_20260903 ── ORV chip retired; historical ORV runs list under ALL */ => ( /* ── RUN_FAVOURITES_20260830 ── CBO/BRK chips */ /* ── VAP_BT_UI_HIDE_20260827 ── VAP_V1 filter chip removed; historical VAP runs still list under ALL */
            <button key={sId}
    ...
    <div>
      ...
      {mode === "table" ? (
        <RunsTable
          ...
          onDelete={del} onOpenRun={onOpenRun} onMeta={setMeta}   /* ── RUN_FAVOURITES_20260830 ── */
...
/* ── RUN_FAVOURITES_20260830 ── one-line note under Key params. Click the text (or ✎) to
...
function RunsTable({
  ...
  onMeta,                                 // ── RUN_FAVOURITES_20260830 ── (rid, {favourite?, note?})
}) {
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", ...typography.bodyMedium }}>
        <thead style={{ background: c.bg.tertiary }}>
          <tr>
            ...
            {th("favourite", "★", "center")}   {/* ── RUN_FAVOURITES_20260830 ── */}
          ...
          <tr>
            ...
            <th style={{ borderBottom: `2px solid ${c.border.light}` }} />   {/* ── RUN_FAVOURITES_20260830 ── ★ column */}
            <th style={{ borderBottom: `2px solid ${c.border.light}` }} />
            <th style={{ borderBottom: `2px solid ${c.border.light}` }} />
            <th style={{ borderBottom: `2px solid ${c.border.light}` }} />
          ...
          {rows.map((r, i) => {
            ...
            return (
              <tr key={r.run_id}
                ...
                {/* ── RUN_FAVOURITES_20260830 ── star toggle */}
                ...
                <td style={{ padding: "8px 10px", fontSize: 11, color: c.text.secondary, maxWidth: 320 }}>
                  {keyParams || "—"}
                  {/* ── RUN_FAVOURITES_20260830 ── inline note editor */}
